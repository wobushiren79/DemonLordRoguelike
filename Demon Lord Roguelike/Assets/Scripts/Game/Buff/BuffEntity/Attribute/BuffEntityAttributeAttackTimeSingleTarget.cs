/// <summary>
/// 单体定向攻速 BUFF：选取时随机锁定一只防守生物，仅对该生物按 trigger_value_rate 缩放攻击时间。
/// <para>用于「急性子」深渊馈赠——随机一只防守魔物 攻击间隔减半(trigger_value_rate=0.5) 即攻击翻倍快。</para>
/// <para>攻速不走属性管线，由 BuffHandler.ChangeAttackTimeDataForBuff 扫描深渊馈赠池，并据 <see cref="IBuffSingleTarget"/>
/// 限定到锁定的那只生物；因只在取攻击时间时临时缩放、不改持久 CreatureBean，故不污染存档。可重复选取(level=0)叠加。</para>
/// </summary>
public class BuffEntityAttributeAttackTimeSingleTarget : BuffEntityAttributeAttackTime, IBuffSingleTarget
{
