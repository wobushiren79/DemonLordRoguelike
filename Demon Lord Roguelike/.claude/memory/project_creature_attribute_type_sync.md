...
**How to apply**：
- 修改装备属性池或 `CreatureAttributeTypeEnum` 时，核对配置表 id 覆盖 1~13 全部入池枚举（目前缺行风险位：MPF=11 已于 2026-08-30 补齐，CMP=12 仅作 BUFF 标签不入装备池未补——配置表与语言表均无 id=12，查询返回 "???" 属预期不报错）。
- 属性面板支持 id 1~10、11、13 的显示与颜色（color_text 解析）。语言表 `Language_CreatureAttributeTypeInfo_*` 12 语种 id=11 均已存在。
- ⚠️ 除「配置行缺失」外还有「行在但语言文本缺失」的变体：CDMG=13 配置行 2026-09 前就存在（name=13）但 `excel_language[多语言_FrameWork].xlsx` 的 CreatureAttributeTypeInfo 表缺 id=13 语言行 → 查询其名时 LogError「没有找到类名为CreatureAttributeTypeInfo ID为13的文本」（2026-09-12 由战斗测试「防守方固定属性」面板全枚举查询暴露，已补齐 12 语种 id=13=暴击伤害/CDMG）。排查同类症状时两步都查：配置表行 + 语言表行。
