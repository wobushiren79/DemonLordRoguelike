...
**风摆粒子 4 shader 落地结构**：`Shader_Particle_{Grass,Tree}WindSway_{Unlit,Lit}.shader`；include 链 `.shader → Effect/WindSway/{Grass,Tree}Wind.hlsl → ../../Common/ParticleCommon.hlsl`。改通用淡出=改 `ParticleCommon`(1处全生效)；改草/树风摆=改对应 `*Wind.hlsl`(1处, Unlit+Lit 同步)。注：`Shader_Particle_GrassWindSway_Lit` 除 UniversalForward 外已补 **ShadowCaster + DepthOnly** 两个 pass（同样 include GrassWind.hlsl 复用 `ApplyWind` + 贴图 alpha `clip`，投草形阴影/写深度；不加 URP 的 ApplyShadowBias），其余 3 个 shader 仍仅 Properties + vert/frag。草粒子受光/阴影整套踩坑见 [[reference_grass_particle_lit_shadow]]。
