...
- [reference_world_quick_attack_research.md](reference_world_quick_attack_research.md) — 战斗进度条 Quick 按钮(+10%进度并即时刷该段进攻波次) + Speed2 按钮(RadioButton 整场2倍速,默认关,仅本场有效;SetGameSpeed 改 fightData.gameSpeed 非 TimeScale,AI/弹道/掉落统一走 GameFightLogic.GetFightDeltaTime,动画 SetAnimTimeScale=SkeletonAnimation.timeScale 同步)；id 块约定 1003_10_W_nn 仅是 Excel 填表规范(难度nn12~20链式/Quick nn30~39/2倍速nn40~49)，起始id存世界表 unlock_id_conquer_difficulty_level/quick_attack/speed2 列,代码只做 起始id+(难度-1),起始id=0返回-1；Quick节点显示前置走 pre_data 列(World1ConquerCompleteCount{难度}:1 通关才显示, Speed2 留空靠 pre_unlock_ids 把关)；研究节点 research_type=4 世界分类
...
- [reference_unity_mcp_execute_code_encoding.md](reference_unity_mcp_execute_code_encoding.md) — MCP execute_code 中文路径乱码([????].xlsx)根因=PS5.1 双引号剥离+Invoke-WebRequest 按 ISO-8859-1 发 body(均已修,用 invoke-unity-mcp.ps1 -BodyBase64)；GetExcelPackage 静默吞异常致导出/生成"假成功"；加列后须先 CreateEntities→编译→再导 JSON(导出按程序集 Bean 反射序列化)
- [reference_volumetric_light_shaft.md](reference_volumetric_light_shaft.md) — 体积光柱不可见三因：URP Spot 光强是 Candela 物理量级(3cd≈不可见,月光柱要 1000cd 级)、雾高须覆盖光柱路径(灯 Y10 则雾高≥12)、enableAdditionalLightsContribution 默认关须显式开；VolumetricAdditionalLight 参数调法(先 Scattering 后光强)
- [reference_instanced_lit_flat_gi.md](reference_instanced_lit_flat_gi.md) — Graphics.DrawMeshInstanced 的 Lit 材质比预制 MeshRenderer 偏暗=缺环境光(SampleSH 读不到环境探针)+缺整份主光(URP 主光 uniform 缺失,实测主光归零弹体无变化)+缺附加光；CustomProvided+CopySHCoefficientArraysFrom 灌 per-instance SH **无效**(自定义 shader 未启用 UNITY_INSTANCED_SH)；确定性修法=MPB 灌 _InstancedFlatGI/SH(按桶 ambient 两方式) + _InstancedLightDir/_InstancedLightColor 模拟主光(Flat/SH 两桶共灌,面片桶恒定提亮/3D桶恢复明暗)；属性走 MPB 不写材质(预制共用会双亮)；⚠️MaterialPropertyBlock 禁在 MonoBehaviour 构造期/字段初始化器 new(CreateImpl 异常连带组件创建失败)→运行时懒建
- [reference_unity_mcp_execute_code_assets.md](reference_unity_mcp_execute_code_assets.md) — 用 Unity MCP execute_code 建资源/特效踩坑：编译器是 CodeDom(C#6)不能 C#7+ 且包编辑器程序集(Addressables)不被引用须反射调；**中文字面量会被编译破坏→路径用 Directory.GetFiles 通配符枚举(全ASCII模式)**；**校验新.cs编译=反射Assembly-CSharp.GetType**；AddressableAssetSettingsDefaultObject 在 UnityEditor.AddressableAssets、Group/Settings/ModificationEvent 在 .Settings；safety_checks 拦 DeleteAsset(关它会被拒)→用 load-or-create；⚠️无人值守(Unity非前台)时 execute_code 做 AssetDatabase 写会死锁主线程(建资产优先改用自跑编辑器脚本)；本项目特效走 Addressables 组 Effect、address=全资源路径,新特效必须注册否则 LoadAssetSync 返 null；EffectBase(mainPS+listPS,PlayEffect连带子PS)+EffectBean(isPlayInShow=false回调里改startColor再播=运行时按稀有度上色)；PS 调 MCP streamable-http 每调用重握手+session id 数组要强转字符串
...
- [`feedback_text_replace_enum.md`](feedback_text_replace_enum.md) — 文本含动态数值一律用 TextReplaceEnum 占位符 + GetTextReplace 通用机制（{Value}/{KillNum} 等语义占位），禁止 string.Format 特判或数值写死进文本；数值源须单一真实源
