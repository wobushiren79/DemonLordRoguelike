...
- [reference_juicer_system.md](reference_juicer_system.md) — 魔汁机(魔物回收/Juicer)系统：术语统一(面向玩家魔汁机/代码Juicer)+UI驱动轻量Logic架构(E键JuicerInteraction=9直接开UICreatureJuicer→选单目标魔物复用CreatureAscendTarget卡片态→Start走GameHandler.StartCreatureJuicer→CreatureJuicerLogic.StartJuice)；建筑ScenePrefabForBase.objBuildingJuicer门控UnlockEnum.Juicer=100600001；⚠️榨汁流程/奖励留桩待接入StartJuice；配置ResearchInfo/UnlockInfo/Language(2009魔汁机/61010提示,icon占位)；详见juicer-system skill+game-juicer agent
