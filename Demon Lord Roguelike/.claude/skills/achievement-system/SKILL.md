...
- **不再有 parent_id/level**。每行用 **两列逗号分隔字符串**承载各级数据：
  - `target_values`：各级目标值，如击杀 `"10,100,1000,5000,10000,50000,100000,500000,1000000,10000000"`（顺序即等级 1..N）
  - `reward_crystals`：各级奖励魔晶，与 target_values 一一对应，如 `"10,50,200,500,1000,5000,10000,20000,50000,100000"`
...
- **一行一个成就**：类型1(击杀)1行(10级)；类型2(时长)1行(10级)；类型3(征服)**合并为 1 行**(全难度合计 1/10/100/1000/10000 次 = 5级)；类型4(征服某世界·按已通不同难度数)1行(10级)；类型5(扭蛋种族抽出)7 行(每种族 1 行 5 级)。总计 **11 行 = 11 张卡**。
...
- `100001` 击杀类（1 行 10 级：target_values=`10,100,1000,5000,10000,50000,100000,500000,1000000,10000000`，reward=`10,50,200,500,1000,5000,10000,20000,50000,100000`）
