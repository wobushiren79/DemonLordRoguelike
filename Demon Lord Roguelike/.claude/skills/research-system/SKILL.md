...
| `pre_data` | string | 前置解锁条件（特殊），`ResearchPreConditionEnum` 枚举名:数值，`&` 组合（与），详见下方「pre_data 前置解锁条件」 |
...
// pre_data 前置条件解析(缓存)：拆分走 StringExtension.SplitForDictionaryEnumLong(枚举名:数值, &组合, 数值缺省1)；无法识别的条目记 isPreDataParseError
public Dictionary<ResearchPreConditionEnum, long> GetPreDataConditions();

// pre_data 前置条件是否全部满足(无配置恒true; 解析出错恒false, 用节点隐藏暴露配置错误)
...
## pre_data 前置解锁条件（特殊条件）

`pre_unlock_ids` 只能表达「解锁 id 前置」，无法表达「通关某难度 N 次」这类特殊条件。`pre_data` 列承接这类需求，与 `pre_unlock_ids` **一起判定**（`UIBaseResearch.CheckPreIsUnlock`：两者全满足节点才显示）。

**格式**：`条件枚举名:数值`，多条用 `&` 组合（与关系），数值缺省为 1（沿用 `NumberBean` 的 `&`/`:` 解析约定）。

```
World1ConquerCompleteCount2:1                          // 世界1难度2通关次数 >= 1
...
**注意**：
- `pre_data` 字段目前在 `ResearchInfoBeanPartial.cs`（主 Bean 是自动生成文件不可直改；导出器按表头名反射 `GetField`，Partial 字段同样能被导出/反序列化）——下次重新生成 ResearchInfo Entity 后删除 Partial 里的临时字段。
- 解析出错（枚举名拼错/填了数字）时 `CheckPreDataIsMeet` 恒 false，节点直接隐藏以暴露配置错误，同时 `LogUtil.LogError` 输出问题条件。
...
| 研究/解锁/前置条件枚举 | `Assets/Scripts/Enums/GameStateEnum.cs` |
