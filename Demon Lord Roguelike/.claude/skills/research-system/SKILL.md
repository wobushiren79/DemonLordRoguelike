...
研究模块是基地科技树系统，玩家在基地（`UIBaseCore`）通过消耗水晶逐级研究节点，解锁设施、属性强化与魔物相关功能。研究节点之间存在前置依赖关系，通过连线（`ui_Line`）可视化展示，并按 **设施 / 强化 / 魔物 / 世界** 四大类型分页切换。
...
### 世界分支(1003_10_W_nn 段) — 每个世界的「加快进攻节奏(Quick)」研究

「加快进攻节奏」是**世界分类(research_type=4)**里的解锁开关型研究，与**世界绑定**：解锁某世界的该研究后，玩这个世界的征服战斗时，战斗界面进攻进度条上的 **Quick 按钮**才显示；点击 Quick 立即向前推进 10% 进攻进度并生成该时间段的进攻生物（详见 [`game-fight-system`](../game-fight-system/SKILL.md) 的 `QuickAdvanceAttackCreate`）。
...
| 01 | 世界解锁 | 100310101 |
| 02 | 无尽模式解锁 | 100310102 |
| 12~20 | 征服难度(等级2~10, 难度研究 `level_max=9` 占满该段) | 100310112(起始)~100310120 |
| **30** | **加快进攻节奏(Quick)** | **100310130** |

> ⚠ Quick 偏移取 **nn=30**，刻意避开征服难度占用的 `12~20` 段。**新增世界时无需给世界配置表(excel_game_world_info)加列**——按约定推导即可：`unlock_id = 100310000 + worldId*100 + 30`（world2→100310230，world3→100310330…）。

**落表**（world1，其它世界照此在各自块内加 nn=30 的一行）：
- `excel_research_info`：id=100310130, `research_type=4`, `icon_res=ui_research_30`, `level_max=1`, `unlock_id=100310130`, `pre_unlock_ids` 留空(世界页直接可见), `pay_crystal=2000`, `name=100310130`, 备注「剑与魔法-加快进攻节奏」
- `excel_unlock_info`：id=100310130, `unlock_type=0`
- `excel_language` 的 `ResearchInfo` 工作表：id=100310130, cn「加快进攻节奏」/en「Faster Assault」

**代码判定**（`UserUnlockBean`，见下方「解锁数值获取」区）：
```csharp
// id 块基址与 Quick 偏移集中在 UserUnlockBean 常量，改约定只改这里
public const long WORLD_UNLOCK_BLOCK_BASE = 100310000;
public const int  WORLD_QUICK_ATTACK_UNLOCK_OFFSET = 30;
public long GetWorldQuickAttackUnlockId(long worldId);   // = BASE + worldId*100 + 30
public bool CheckIsUnlockWorldQuickAttack(long worldId); // 战斗中按当前 worldId 判定是否显示 Quick 按钮
```
...
public RadioButtonView ui_UIViewCommonLabel_World;           // Tab: 世界
...
public void RadioButtonSelected(RadioGroupView rg, int position, RadioButtonView rb)
{
    ...
    else if (rb == ui_UIViewCommonLabel_World)
