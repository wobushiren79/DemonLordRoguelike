...
> **卡片详情回复型显示**（`UIViewCreatureCardDetails.SetAttribute`）：回复型生物（攻击方式 class_name 为 `AttackModeRegain` 系，见 attack-mode-system）隐藏攻击力条目 `ui_ViewCreatureCardItemAttribute_Atk`，按回复子类二选一改显：加血型（`creatureInfo.IsRegainHPAttackMode()`，`AttackModeRegainHP` 系，攻击模式 id=500001）显示治疗量条目 `ui_ViewCreatureCardItemAttribute_AddLife`，加护甲型（`IsRegainDRAttackMode()`，`AttackModeRegainDR` 系，id=500002）显示回甲量条目 `ui_ViewCreatureCardItemAttribute_AddDef`（值均 = 当前ATK×攻击模式 `GetDamageAddRate()`，与战斗实际恢复量口径一致）；非回复型隐藏 AddLife/AddDef 条目、正常显示攻击力。魔王固定隐藏 AddLife/AddDef 两项（SetAttribute 开头统一先隐藏，防详情面板池化复用残留）。
