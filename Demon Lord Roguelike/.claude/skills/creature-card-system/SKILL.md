...
> **动态属性馈赠（都是兄弟/杀红了眼）天然被展示判定通过**：它们继承 `BuffEntityAttribute`（即 `IAttributeModifierSource`），故 `IsAbyssalBlessingTargetCreature` 三连（属性BUFF + 生物类型 + 单体定向 UUID）自然放行，**无需改判定逻辑**。展示范围随作用范围：**都是兄弟**（`trigger_creature_type=1` 作用全体防守）与其它全体防守馈赠一样每张防守卡显示一份；**杀红了眼**兼实现 `IBuffSingleTarget`，仅在选取时随机锁定的那只防守魔物卡上显示一份图标（按锁定 UUID 精确匹配）。卡片详情面板 `GetAttribute(true)` 同理天然叠加其动态率。
