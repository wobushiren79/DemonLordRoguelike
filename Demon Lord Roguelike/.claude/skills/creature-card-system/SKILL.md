...
| `ShowNoPopup` | 展示但不弹详情 |
| `Fight` | 战斗场景 |
| `Lineup` | 阵容行（阵容管理界面上方横排，拖拽换位） |
| `LineupBackpack` | 阵容背包（阵容管理界面的生物列表，不可拖拽） |
...
> **卡片详情属性显示配置化 + 回复型显示**（`UIViewCreatureCardDetails.SetAttribute`）：按 `creatureInfo.GetShowAttributeList()`（`show_attribute` 列，逗号分隔枚举值，空=默认 `1,3,4,6`，见 creature-system）开关槽位——HP→Life/DR→Def/ATK→Atk/ASPD→Speed/MSPD→Speed/MP→MP/MPF→MPR 槽，未配置的属性隐藏（如烂泥/毒液史莱姆 3003/3004 配 `4` 只显示攻击力）；Speed 槽 ASPD/MSPD 共用（同时配置优先攻速），MPR(魔力回复%) 无槽位忽略；8 槽每次全部显式 SetActive 防池化残留，AttributeList 容器 GridLayoutGroup+ContentSizeFitter 自动收紧。其上回复型生物（攻击方式 class_name 为 `AttackModeRegain` 系，见 attack-mode-system）配置含 ATK 时不显攻击力条目 `ui_ViewCreatureCardItemAttribute_Atk`，按回复子类二选一改显：加血型（`creatureInfo.IsRegainHPAttackMode()`，`AttackModeRegainHP` 系，攻击模式 id=500001）显示治疗量条目 `ui_ViewCreatureCardItemAttribute_AddLife`，加护甲型（`IsRegainDRAttackMode()`，`AttackModeRegainDR` 系，id=500002）显示回甲量条目 `ui_ViewCreatureCardItemAttribute_AddDef`（值均 = 当前ATK×攻击模式 `GetDamageAddRate()`，与战斗实际恢复量口径一致）。魔王同样走配置（其物种行 id 1-7 配 `4,5,2,11` = ATK/MSPD/MP/MPF），无特判分支。
...
> **动态属性馈赠天然被展示判定通过**：动态率馈赠 BUFF 继承 `BuffEntityAttribute`（即 `IAttributeModifierSource`），故 `IsAbyssalBlessingTargetCreature` 三连（属性BUFF + 生物类型 + 单体定向 UUID）自然放行，**无需改判定逻辑**。展示范围随作用范围：随魔物数缩放类（`trigger_creature_type=1` 作用全体防守，曾用于都是兄弟）与其它全体防守馈赠一样每张防守卡显示一份；随击杀数缩放类（兼实现 `IBuffSingleTarget`，曾用于杀红了眼）仅在选取时随机锁定的那只防守魔物卡上显示一份图标（按锁定 UUID 精确匹配）。卡片详情面板 `GetAttribute(true)` 同理天然叠加其动态率。（⚠️ 现役无动态率馈赠配置，机制留存。）
...
> **魔王卡片的特殊渲染（卡片项 `UIViewCreatureCardItem` 与详情 `UIViewCreatureCardDetails`）**：以 `CreatureBean.IsDemonLord()`(单一真实源) 判定后差异化展示——① **稀有度统一按魔王专属档 `RarityEnum.DemonLord`(=999) 配置显示**(`SetRarity((int)RarityEnum.DemonLord)`，仅显示取 `RarityInfo` id=999 的深黑+暗紫红配色，不改存档 `rarity`；999 是魔王的标记值、非真实档位，不参与稀有度排序/进阶/BUFF/CMP 等数值比较)；② **隐藏等级**：卡片项 `SetLevel(level, isHide)` 新增 `isHide` 参数隐藏 `ui_LevelText`；详情面板在 `SetData` 里对魔王隐藏**等级容器 `ui_Level`** 与**详情基础容器 `ui_DetailsBase`**(`SetLevelData` 对魔王直接 return 不赋值)；③ 详情 `SetMP` 对魔王隐藏 `ui_MP`(魔王无召唤耗魔概念)；④ **属性列表分流**(`SetAttribute()`)：所有生物统一走 `show_attribute` 配置开关槽位——魔王物种行(id 1-7)配 `4,5,2,11` 故显示 ATK/MSPD/MP/MPF（MSPD 复用 Speed 槽位、MPF 显示在预制体命名 MPR 的项上），普通生物默认 `1,3,4,6`（回复型配置含 ATK 时换显 AddLife/AddDef，见上文「卡片详情属性显示配置化 + 回复型显示」）。`UIViewCreatureCardDetails.IsDemonLord()` 已收口为委托 `creatureData.IsDemonLord()`。⑤ **Details 面板整体空隐藏**(`RefreshUILayout`，SetData 末尾调用)：遍历 `ui_Details` 全部直接子容器(DetailsBase/NameDoomCouncil/RenmarkText/Details_Child_1/Level/Equip/Buff)，全部 inactive 时 `ui_Details` 一起 SetActive(false)（如魔王：等级/详情基础/称号关系均隐藏，装备与BUFF也为空则整个右侧面板不残留）；任一子容器显示则恢复显示。注意属性列表 AttributeList 在 `ui_Details` 之外，不参与该判定。
