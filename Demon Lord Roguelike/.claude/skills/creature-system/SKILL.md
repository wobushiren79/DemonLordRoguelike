...
> **幻化（`CreatureBean.transformItemId`，long 持久化字段）**：0=无幻化；幻化药（`ItemTypeEnum.TransformPotion`=18，消耗品）使用后写入对应道具 id，幻原药（`RestorePotion`=19）置0 清除恢复原形象；连续吃幻化药后者覆盖前者（直接覆写 id）。随存档 JSON 序列化（旧存档无此字段默认0兼容）；**只存 id 不存资源名**——展示时实时查 `ItemsInfoCfg`，Mod 提供的幻化药被移除→配置 null→全路径自动回落原形象，装回自动恢复；幻原药流程只判 id==0 不读配置，Mod 移除后残留 id 仍可正常清除。`ClearTempData()` 重置为0（防一次性 Bean 入池复用残留），`ClearFightTempData()` 不动（战斗结束幻化保留）。**解析入口 `CreatureBeanPartial.GetTransformSpineRes()`**（`#region 幻化相关`，全项目唯一形象解析入口）：transformItemId=0 / 配置缺失（静态 `loggedMissingTransformIds` 每 id 每会话只 LogError 一次，防列表刷新刷屏）/ 道具类型非 TransformPotion（防 Mod id 被复用成其它道具）/ `other_data` 为空 均返回 null（调用方回落原形象），否则返回 `ItemsInfo.other_data` 的 **chess 段**（组合格式 `chessRes,avatorRes|uiScale;x,y`，由 `ParseTransformOtherData` 解析；资源名=Spine SkeletonDataAsset 的 Addressables 资源名/Mod catalog key）。⚠️ 幻化目标资源须含完整动画集（Idle/Walk/Attack/Dead）。**高清展示（ui_show_spine）**：`GetTransformUIShowSpineRes()` 取 avator 段（可空，空=详情UI也用 chess 段），`CreatureHandler.SetCreatureData` 在 `isUIShow=true` 时优先改用 avator 资源；`GetTransformUIShowData()` 取 `|` 第三段详情UI尺寸（格式同 `ui_data_b`，按 Mod Avator 骨架校准），`GameUIUtil.SetCreatureUIForDetails` 用它替代原生物 `ChangeUISizeForB`。**形象优先级链：Portrait 装备 > 幻化 > 原形象**。Mod 幻化药生产实例见 aeonsecho-spine-mod Skill。
...
public partial class FightCreatureEntity
{
    ...
    public void RegainDR(BaseAttackMode attackMode);     // 回复护甲（真实加甲>0 时播攻击模式配置的命中音效 sound_hit，加甲型 500002 配 sound_medicine_1=470001，与治疗一致）
...
> **`show_attribute`(string) — 展示属性列表（详情显示/献祭加点/创建随机加点 三处共用）**：配置列在 `excel_creature_info` 末尾（col46），逗号分隔 `CreatureAttributeTypeEnum` 枚举值（如 `1,3,4,6`=HP/DR/ATK/ASPD，`4`=仅攻击力）。自动生成 `CreatureInfoBean.show_attribute`；手写解析 `CreatureInfoBeanPartial.GetShowAttributeList()`（懒解析缓存；空/解析失败/为空统一兜底默认 HP/DR/ATK/ASPD，旧 Mod 无此键兼容）。三处消费方：① 详情面板 `UIViewCreatureCardDetails.SetAttribute` 按列表开关 8 槽位（回复型 ATK 槽仍映射 AddLife/AddDef；魔王同样走配置，其物种行 id 1-7 配 `4,5,2,11`=ATK/MSPD/MP/MPF）；② 献祭加点界面 `UICreatureAddAttribute.InitItems` 按列表显隐 4 个固定加点项（含 4 项以外属性告警忽略，全隐藏兜底强制显示 HP 项）；③ 创建随机加点 `CreatureAttributeBean.AddRandomAttributeForCreate(addNum, pool)` 随机池（`CreatureBeanPartial.RandomAttributeForCreate` 传 `creatureInfo?.GetShowAttributeList()`）。当前配置：烂泥/毒液史莱姆（3003/3004）= `4`（不被攻击只有攻击力有用）、守护史莱姆（3001）= `1,3`（无攻击模式纯肉盾只显 HP/DR）、魔王物种行（id 1-7）= `4,5,2,11`，其余全部生物 = `1,3,4,6`。
