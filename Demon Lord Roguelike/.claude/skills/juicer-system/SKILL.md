---
name: juicer-system
description: Demon Lord Roguelike 游戏的魔物回收(魔汁机/Juicer)系统开发指南。使用此SKILL当需要创建或修改魔汁机建筑出现/解锁显隐、魔汁机E键场景交互(打开UICreatureJuicer)、选目标魔物榨汁UI、榨汁逻辑(CreatureJuicerLogic)、魔汁机研究解锁(UnlockEnum.Juicer)、以及后续接入榨汁流程与奖励等，包括 ScenePrefabForBase.objBuildingJuicer/BuildingJuicerRefresh/AnimForBuildingJuicerShow、ControlInteractionEnum.JuicerInteraction、UICreatureJuicer(单选目标+Start)、CreatureJuicerLogic.StartJuice(留桩)、GameHandler.StartCreatureJuicer、excel_research_info/excel_unlock_info/excel_language 配置等。注意：榨汁流程与奖励目前为留桩，后续接入 StartJuice。
watched_files:
...
目前只搭好**入口骨架**：`解锁 → 建筑出现 → E键交互 → 开UI选目标魔物 → 点Start进逻辑`。
**真正的榨汁流程（魔汁机建筑动画 / 消耗目标魔物）与奖励结算尚未实现，留桩在 `CreatureJuicerLogic.StartJuice`**，后续需求接入时优先在此按 TODO 补齐，不要打散到 UI 层。
...
基地场景魔汁机建筑 objBuildingJuicer
  ...
  └─ 玩家靠近按 E → ControlForGameBase.HandleForUseEUp
       ...
       → UIHandler.OpenUIAndCloseOther<UICreatureJuicer>(ui =>
            ...
            └─ UICreatureJuicer: 选单个目标魔物(单选) → 点 Start
                 → GameHandler.Instance.StartCreatureJuicer(target)
                      → CreatureJuicerLogic.StartJuice(target) //【留桩】榨汁流程/奖励后续接入
            退出(ui_ViewExit) → actionForExit() → 回 UIBaseMain(场景)
```
...
- 判定解锁：
  ...
  ```
- 研究节点 `excel_research_info[研究信息].xlsx` → `ResearchInfo.txt`（设施类照孕育根节点 100400000 结构）：
  `research_type=1`、`unlock_id=name=id=100600001`、`pre_unlock_ids="100500001"`(前置=成就)、`pay_crystal="5"`、`level_max=1`、`icon_res="ui_research_57"`(**占位图标，待替换专属**)、`position_x/y` 后续可调
- 解锁项 `excel_unlock_info[解锁信息].xlsx` → `UnlockInfo.txt`：`unlock_type=0`、`remark="开启魔汁机"`、`id=100600001`
...
- **目标魔物列表**：`InitCreatureData()` 取背包内 **`CreatureStateEnum.Idle` 且未上阵(`CheckIsInAnyLineup`)** 的魔物
- **单选**：`targetCreatureSelect`，`EventForCardClickSelect` 里再次点已选则取消、否则切换目标
- **卡片使用态复用 `CardUseStateEnum.CreatureAscendTarget` + `CardStateEnum.CreatureAscendSelect/NoSelect`**——因为预制体 `ui_UIViewCreatureCardList_Target` 的卡片变体是 `UIViewCreatureCardItemForCreatureAscend`（与容器目标列表同款）。**改卡片变体/状态前先确认预制体挂的脚本**，避免选中高亮失效（选中高亮由各卡片变体 `RefreshCardState` 点亮 `ui_SelectBg`，基类只负责复位）
- 选择事件走 `EventsInfo.UIViewCreatureCardItem_OnClickSelect`（`this.RegisterEvent`，实例事件）
- Start(`OnClickForStart`)：未选目标 → `ToastHintText(TextHandler.GetTextById(61010) "请选择要榨汁的目标魔物")` 拦截；已选 → `GameHandler.Instance.StartCreatureJuicer(targetCreatureSelect)`
- **Start 按钮文本** `ui_BtnStartText`（`UITextLanguageView`）：预制上 `textId = 61011` "开始榨汁"/"Begin Juicing"（无代码赋值，`UITextLanguageView.OnEnable` 自动按 id 取文本）
- `OpenUI` 关基地移动控制 `GameControlHandler.Instance.SetBaseControl(false)`（与其它基地子界面一致）
...
- 字段：`targetCreature`（本次目标魔物）、`scenePrefab`（基地场景预制，榨汁动画将作用于 `scenePrefab.objBuildingJuicer`）
- `StartJuice(CreatureBean target)`：记录目标 + `WorldHandler.Instance.GetCurrentScenePrefab<ScenePrefabForBase>(GameSceneTypeEnum.BaseGaming)`；**当前仅 `LogUtil.Log` 打点，TODO 留桩**：
  ```
  //1.锁UI/切魔汁机镜头; 2.播 objBuildingJuicer 榨汁动画(消耗 targetCreature);
  //3.结算奖励入账 + 存档; 4.反馈提示并返回。
  ```
- `GameHandler.StartCreatureJuicer(CreatureBean target)`：get-or-create `CreatureJuicerLogic` → `StartJuice(target)`（与 `StartGashaponMachine` / `StartCreatureSacrifice` 同风格，均为 `GameHandler.Instance.Start*`）

## 多语言

| 文本 | 文件(真实源=excel_language 同名工作表) | id | cn | en |
|------|------|----|----|----|
| 研究节点名 | Language_ResearchInfo_cn/en | 100600001 | 开启魔汁机 | Unlock Juicer |
| 交互提示 | Language_UIText_cn/en | 2009 | 魔汁机 | Juicer |
| 未选目标提示 | Language_UIText_cn/en | 61010 | 请选择要榨汁的目标魔物 | Please select a monster to juice |
...
- **榨汁流程/奖励统一收进 `CreatureJuicerLogic.StartJuice`** 后续接入，别散到 UI。
- 建筑显隐**唯一门控** `UnlockEnum.Juicer`；交互碰撞体命名固定 `JuicerInteraction`。
- UI 驱动：E键直接开 UICreatureJuicer（不像献祭那样 Logic 驱动开UI）。
- 目标列表卡片态**复用 CreatureAscend 系列**（预制体已如此接线）。
- 配置改 **Excel 唯一真实源**并同步 JSON；自动生成 Bean/JSON 不手改结构（Bean 扩展写 Partial）。
- UI 继承 `BaseUIComponent`；输入走 `InputActionUIEnum`（禁用旧版 Input API）。
- `icon_res` 目前占位（复用孕育图标），需专属魔汁机研究图标时**先征得用户同意再用 PixelLab**。
- 预制体（objBuildingJuicer 交互碰撞体、UICreatureJuicer 接线）本次由用户手动接好；改动涉及预制体时优先走 Unity MCP 或提示用户手工处理。
