...
### FightTypeChallengeHundredInfoBean（挑战100勇士配置, Bean 自动生成禁改 / Partial 手写扩展）
`Assets/Scripts/Bean/MVC/Game/FightTypeChallengeHundredInfoBean(Partial).cs`（Cfg.fileName = `FightTypeChallengeHundredInfo`，一行一难度，当前10行）。
