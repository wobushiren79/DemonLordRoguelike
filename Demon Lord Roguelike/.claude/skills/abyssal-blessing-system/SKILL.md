...
| 闪电（5 级链） | 2000002001~005 | 多级升级链（首个**周期型**馈赠） | 每 3 秒落下 1~5 道闪电随机击中敌人（0.1 秒间隔连发、可重复命中同一目标），落点 0.2~1 半径 AOE，单雷命中上限=等级（1~5 个，超上限按距落点近者优先），伤害=魔王攻击力×BUFF `trigger_value` 倍率；BUFF `3000300001~005`=`BuffEntityPeriodicMultiInstantAttack`（`class_entity_data`="次数,攻击模块ID"，半径/单雷命中上限配在攻击模块表 300031~035 的 collider_area_size/hit_max），落雷粒子走全局单例 `Effect_Thunder_3`(id 900003，`EffectHandler.ShowThunderEffect`) |
