...
当前配置共 **13 行 / 5 族**（由 72 行 20 族精简而来，后新增闪电族）：
...
| 闪电（5 级链） | 2000002001~005 | 多级升级链（首个**周期型**馈赠） | 每 3 秒落下 1~5 道闪电随机击中敌人（0.1 秒间隔连发、可重复命中同一目标），落点 0.2~1 半径 AOE，单雷命中上限=等级（1~5 个，超上限按距落点近者优先），伤害=魔王攻击力×BUFF `trigger_value` 倍率；BUFF `3000300001~005`=`BuffEntityPeriodicMultiInstantAttack`（`class_entity_data`="次数,半径,单雷命中上限"），落雷粒子走全局单例 `Effect_Thunder_3`(id 900003，`EffectHandler.ShowThunderEffect`) |
...
> **已释放 id 段**：单行族 `1000004xxx~1000007xxx`、多级族号 3~13（`2000003xxx~2000013xxx`，族号 2 已被闪电占用）、BUFF 族号 4~8 与 11~20（`30004xxxxx~30008xxxxx`、`30011xxxxx~30020xxxxx`，族号 3 已被闪电占用）。闲置图标 `ui_abyssalblessing_1~5、8、11~14、120~125` 文件仍在图集中未删。
...
- **当前 id 进度**（2026-07 精简后）：现役深渊馈赠 BUFF 段（`buff_type=3`）→ `3000100001`、`3000200001~005`、`3000300001~005`（闪电）、`3000900001`、`3001000001`；现役馈赠多级族号 1（钱多多）、2（闪电），单行族号 1~3；BUFF 族号 4~8、11~20 与馈赠多级族号 3~13、单行族号 4~7 **已释放可复用**（也可避开历史段从新号续编）。彩色图标编号历史用到 125，其中 `ui_abyssalblessing_120~125` 等闲置未删。
