...
| 瓶装炼狱火（5 级链） | 2000009001~005 | 多级升级链（周期型抛物线地形火焰） | 每 10 秒向不放回抽取的 1~5 个敌人投掷炼狱火瓶（第1瓶立即+后续 0.2 秒间隔连发；敌人少于瓶数只丢同等数量——同轮多瓶不瞄同一目标；场上无存活敌人本轮不触发），瓶子抛物线飞向该目标当前位置（**固定落点、不追踪**，目标中途死亡/移动照飞原落点），落地燃放半径 1.2 地形火焰持续 5 秒，每 1 秒对火焰上敌人造成魔王实时攻击力×`trigger_value`(0.1)伤害（保底1、不暴击、不递减——多片叠加可多次跳伤）；BUFF `3001200001~005`=`BuffEntityPeriodicAttackFireBottle`（`class_entity_data`="瓶数,攻击模块ID"），5 级共用攻击模块 300101=`AttackModeRangedArcGround`（双状态：飞行段抛物线不追踪+纯投掷物禁用命中+弹体自旋-720°/s绕-Z轴，燃烧段每1秒 CheckHitTargetArea 半径1.2跳伤、满5秒自毁；collider_area_size 第1项=燃烧半径；飞行段 DSP 火瓶贴图 `AttackModeVisual_FireBottle_1`(贴图复用馈赠图标)，燃烧段 visualBucketKey 置空隐藏 DSP 改走 `EffectHandler.ShowFloorFireEffect` 全局单例 burst 重播粒子 `Effect_FloorFire_1`(1800001)，粒子时长同步燃烧5秒），图标 `ui_abyssalblessing_fire` |
