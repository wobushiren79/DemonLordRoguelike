...
### 现役配置清单（2026-07 精简后，2026-07 新增第六次冲击族）

当前配置共 **43 行 / 11 族**（由 72 行 20 族精简而来，后新增闪电族、失控的矿车族、死亡回旋族、跳跳斧族、知识的力量族、回弹菱块族、第六次冲击族）：

| 馈赠 | id | 类型 | 效果 |
|------|----|------|------|
| 增殖 | 1000001001 | 可重复·不限（`level=0,max_count=0`） | 随机复制一个已有的魔物（即时 BUFF 克隆） |
| 奖励多多 | 1000002001 | 可重复·不限 | 通关奖励数量+1（领奖参数型，见下文） |
| 再来一瓶 | 1000003001 | 可重复·不限 | 通关奖励选择次数+1（领奖参数型，见下文） |
| 钱多多（5 级链） | 2000001001~005 | 多级升级链 | 每次击杀 20%~100% 概率多掉一次魔晶 |
| 闪电（5 级链） | 2000002001~005 | 多级升级链（首个**周期型**馈赠） | 每 3 秒落下 1~5 道闪电随机击中敌人（0.1 秒间隔连发、不放回抽主目标——一轮内多道雷主目标互不重复，敌人少于雷数只发同等数量的雷；溅射不受限：同一目标可被多道雷重复溅射、被溅射过的目标仍可作主目标），落点 0.2~1 半径 AOE，单雷命中上限=等级（1~5 个，超上限按距落点近者优先），AOE 多目标伤害按命中次序依次减半（保底 1），伤害=魔王攻击力×BUFF `trigger_value` 倍率；BUFF `3000300001~005`=`BuffEntityPeriodicAttackMultiInstant`（`class_entity_data`="次数,攻击模块ID"，半径/单雷命中上限配在攻击模块表 300031~035 的 collider_area_size/hit_max），落雷粒子走全局单例 `Effect_Thunder_3`(id 900003，配在攻击模块表 `effect_hit` 列→`EffectHandler.ShowEnduringSingletonEffect`) |
| 失控的矿车（5 级链） | 2000003001~005 | 多级升级链（周期型道路冲撞） | 每 5 秒随机选 1~5 条道路，每条从最左端驶出 1 辆矿车沿路向右碾压（穿透整路、每个目标只撞一次），撞到的第 1 个敌人伤害=魔王攻击力×`trigger_value`，之后每多撞 1 个减半（保底 1），驶到路尽头消失；车数>路数时每路先各 1 辆、多出的随机重复分配，同路第 2 辆起间隔 0.5 秒错开出车；BUFF `3000400001~005`=`BuffEntityPeriodicAttackRoad`（`class_entity_data`="车数,攻击模块ID"），5 级共用攻击模块 300041=`AttackModeRangedPiercingRoad`（速度 10、collider_size 0.3、sound_start=310001(出车音效 sound_clean_1，框架 0.1s 同音去重防同帧多车叠播)、sound_hit=420001、残影拖尾），视觉 `AttackModeVisual_Minecart_1`，图标 `ui_abyssalblessing_minecar` |
| 死亡回旋（5 级链） | 2000004001~005 | 多级升级链（周期型回旋镖） | 每 5 秒从魔王处发射 1~5 枚回旋镖随机瞄准敌人（不放回抽目标——一轮内多镖目标互不重复，敌人少于镖数只发同等数量；第 1 镖立即+后续 0.2 秒间隔连发），伤害=魔王攻击力×`trigger_value`、逐目标减半保底 1，镖到达目标后超出一格折返回到魔王身边（时间参数化速度曲线、返程可再次命中）；BUFF `3000500001~005`=`BuffEntityPeriodicAttackBoomerang`（`class_entity_data`="镖数,攻击模块ID"），5 级共用攻击模块 300051=`AttackModeRangedBoomerang`（速度 5、collider_size 0.3、sound_start=400003(发射音效 sound_fight_3)、sound_hit=420001、残影拖尾、弹体自旋 -720°/s），视觉 `AttackModeVisual_Boomerang_1`，图标 `ui_abyssalblessing_boomerang` |
| 跳跳斧（5 级链） | 2000005001~005 | 多级升级链（周期型弹跳斧头） | 每 5 秒从魔王处扔出 1~5 把斧头，抛物线追踪飞向**随机一排的最远敌人**（按 roadIndex 分组、同路 x 降序；不放回抽取——一轮内多斧首目标互不重复，敌人少于斧数只扔同等数量；第 1 斧立即+后续 0.2 秒间隔连发），伤害=魔王攻击力×`trigger_value`、逐目标减半保底 1；命中后弹跳至附近 1 单位内另一目标（Lv1~5 弹跳 1~5 次，同一斧头目标不重复，无候选即停），弹跳也是抛物线分段（弧高=首段一半，每段仅下落阶段命中）；BUFF `3000600001~005`=`BuffEntityPeriodicAttackBounceAxe`（`class_entity_data`="斧数,攻击模块ID,弹跳次数"），5 级共用攻击模块 300061=`AttackModeRangedArcBounce`（速度 4、collider_size 0.3、弹跳搜索半径配在 collider_area_size 第 1 项=1、sound_start=400003(发射音效 sound_fight_3)、sound_hit=420001、残影拖尾、弹体自旋 -720°/s、单段最短飞行时长 1s），视觉 `AttackModeVisual_Axe_1`，图标 `ui_abyssalblessing_axe` |
| 知识的力量（5 级链） | 2000006001~005 | 多级升级链（首个**常驻环绕型**馈赠） | 随机选取 1 只最前排（x 最大）的己方存活魔物作为宿主，其周围 XZ 水平面环绕 1~5 本书（半径 0.75、高度取魔王位置+攻击起始偏移、转速 Lv1~5=0.6/1.2/1.8/2.4/3 弧度/秒、角度均分），触碰到的敌人受到魔王实时攻击力×`trigger_value`(0.4) 的伤害、不暴击（每只书对同一敌人 0.5 秒冷却）；宿主死亡或出现更前排魔物（x 更大）→ 自动改选当前最前排（并列 x 随机 1 只、并列不换防抖动），场上无己方魔物 → 书本全销毁、重新有魔物再生成；书本=常驻攻击模块 `AttackModeOrbit`（不自毁、不做周期触发，`trigger_time=9999`），关卡切换被回收后下一帧整套重建；BUFF `3000700001~005`=`BuffEntityPeriodicAttackOrbit`（`class_entity_data`="书本数,攻击模块ID,转速"），5 级共用攻击模块 300071=`AttackModeOrbit`（attack_search_type=11 球形接触、collider_size 0.3、sound_hit=420001、残影拖尾），视觉 `AttackModeVisual_Book_1`（复用图标贴图），图标 `ui_abyssalblessing_book` |
| 回弹菱块（5 级链） | 2000007001~005 | 多级升级链（常驻反弹弹球型馈赠） | 场上**永久维持** 1~5 颗菱块（不随时间销毁、不随轮次累积、总数恒定=等级），菱块在道路矩形（x∈[0.5,0.5+路长]、z∈[0.5,路数+0.5]）内四壁反复反弹（越墙钳位+方向分量取反+绕 Y 轴 **±5° 随机偏转**防 90° 死角死循环；左墙在子弹进入场地后才生效防出生即弹回魔王侧），初始角度为以 +x 为 0° 的前向锥 ±75° 随机（保证射入道路范围），场上无敌人也照常发射；击中敌人伤害=魔王实时攻击力×`trigger_value`(1)、不暴击、不递减，**同一目标 1 秒冷却**（每颗弹独立记录各目标最近命中时刻）；弹速 Lv1~5=2/2.5/3/3.5/4 配在攻击模块行 `speed_move`；BUFF `3000800001~005`=`BuffEntityPeriodicAttackRebound`（`trigger_time=1` 每秒自检补弹到 N 颗——关卡切换存量弹被 `ClearAttackModePrefab` 清掉后自动补回，升级替换/清空时 `ClearData` 主动销毁全部存量弹；`class_entity_data`="弹数,攻击模块ID"），攻击模块 300081~300085=`AttackModeRangedRebound`（collider_size 0.6、sound_hit=420001、残影拖尾、弹体不自旋），视觉 `AttackModeVisual_Pingpang_1`（贴图 `Textures/Game/Pingpang_1.png` 复用图标），图标 `ui_abyssalblessing_pingpang` |
| 第六次冲击（5 级链） | 2000008001~005 | 多级升级链（周期型圆环冲击波；首个带**击退机制**的馈赠） | 每 10 秒从魔王位置+攻击起始偏移处发出一道圆环冲击波，以魔王为圆心扩张至覆盖整条道路（最大半径=道路右缘 0.5+路长 +余量 0.5 −圆心x，攻击模块按当场路长计算），扫到的敌人受魔王实时攻击力×`trigger_value`(Lv1~5=0.1~0.3)伤害（保底1、不暴击）并被**击退 0.5**（交 AI 击退意图 `AIIntentAttackCreatureKnockback`：方向固定 +x 沿道路向后推、不带 z 分量防推离路径、固定 0.2s 匀速推完、落点 x 钳制道路范围、攻击循环被打断、结束回闲置重新索敌），每只敌人每波只命中一次，场上无敌人本轮不触发；BUFF `3001100001~005`=`BuffEntityPeriodicAttackShockwave`（`class_entity_data`="攻击模块ID"），5 级共用攻击模块 300091=`AttackModeShockwaveRing`（XZ 环带命中自遍历不走射线批处理，speed_move=6=扩张速度，collider_area_size="余量,击退距离"="0.5,0.5"，sound_start=420001 占位待定），视觉 `Effect_Shockwave_1`(1700001,配在攻击模块表 300091 effect_hit 列) 走 `EffectHandler.ShowEnduringSingletonEffect`（全局单例通道，SingletonEffectParam 的 startSize/startLifetime multiplier 同步半径与扩张时长，视觉波前与判定环带严格重合），图标 `ui_abyssalblessing_boom` |
| 瓶装炼狱火（5 级链） | 2000009001~005 | 多级升级链（周期型抛物线地形火焰） | 每 10 秒向不放回抽取的 1~5 个敌人投掷炼狱火瓶（第1瓶立即+后续 0.2 秒间隔连发；敌人少于瓶数只丢同等数量——同轮多瓶不瞄同一目标；场上无存活敌人本轮不触发），瓶子抛物线飞向该目标当前位置（**固定落点、不追踪**，目标中途死亡/移动照飞原落点），落地燃放半径 1.2 地形火焰持续 5 秒，每 1 秒对火焰上敌人造成魔王实时攻击力×`trigger_value`(0.2)伤害（保底1、不暴击、不递减——多片叠加可多次跳伤）；BUFF `3001200001~005`=`BuffEntityPeriodicAttackFireBottle`（`class_entity_data`="瓶数,攻击模块ID"），5 级共用攻击模块 300101=`AttackModeRangedArcGround`（sound_start=400003(发射音效 sound_fight_3)；双状态：飞行段抛物线不追踪+纯投掷物禁用命中+弹体自旋-720°/s绕-Z轴，燃烧段每1秒 CheckHitTargetArea 半径1.2跳伤、满5秒自毁；collider_area_size 第1项=燃烧半径；飞行段 DSP 火瓶贴图 `AttackModeVisual_FireBottle_1`(贴图复用馈赠图标)，燃烧段 visualBucketKey 置空隐藏 DSP 改走 `EffectHandler.ShowEnduringSingletonEffect(effect_hit=1800001)` 全局单例 burst 重播粒子 `Effect_FloorFire_1`(1800001)，粒子时长同步燃烧5秒），图标 `ui_abyssalblessing_fire` |

> **2026-07 删除 12 族 64 行**：大力出奇迹/膘肥体壮/钢铁憨憨/急性子（单体定向 4 个）、强身健体/时光沙漏/伤害性极强/慢条斯理/唯快不破/坚不可摧（固定数值 6 族 30 行）、都是兄弟/杀红了眼（动态率 6 族 30 行），及其 BUFF、多语言条目。
> **对应 C# 基础设施全部保留**（单体定向/动态率 BUFF 实体类、`IBuffSingleTarget`、广播重算链路等，见下文各节）——当前为"无现役配置引用的留存机制"，新增同类馈赠可直接复用。
> **已释放 id 段**：单行族 `1000004xxx~1000007xxx`、多级族号 10~13（`2000010xxx~2000013xxx`，族号 2 已被闪电占用、族号 3 已被失控的矿车占用、族号 4 已被死亡回旋占用、族号 5 已被跳跳斧占用、族号 6 已被知识的力量占用、族号 7 已被回弹菱块占用、族号 8 已被第六次冲击占用、族号 9 已被瓶装炼狱火占用）、BUFF 族号 9 与 13~20（`30009xxxxx`、`30013xxxxx~30020xxxxx`，族号 3 已被闪电占用、族号 4 已被失控的矿车占用、族号 5 已被死亡回旋占用、族号 6 已被跳跳斧占用、族号 7 已被知识的力量占用、族号 8 已被回弹菱块占用、族号 11 已被第六次冲击占用、族号 12 已被瓶装炼狱火占用）。闲置图标 `ui_abyssalblessing_1~5、8、11~14、120~125` 文件仍在图集中未删。
...
|----|-----------|-------|----------|--------|
| 2000001001 | 0 | 1 | 3000200001 | 钱多多-每次击杀 20% 概率多掉一次魔晶（1级，族根） |
| 2000001002 | 2000001001 | 2 | 3000200002 | 钱多多-40%（2级） |
| 2000001003 | 2000001002 | 3 | 3000200003 | 钱多多-60%（3级） |
| 2000001004 | 2000001003 | 4 | 3000200004 | 钱多多-80%（4级） |
| 2000001005 | 2000001004 | 5 | 3000200005 | 钱多多-100%（5级） |
...
| **一次性**（= N=1 特例） | `0` | `0` | `1` | 整局仅 1 次（当前无现役实例；历史例：大力出奇迹等单体定向馈赠） | 不显示（`level<=0`） |
...
- BUFF 绑定：可重复馈赠按需绑**常驻型**(属性/条件/周期)或**即时型**(`BuffEntityInstant`+计数器，见下文"常驻可重复馈赠")；单体定向类绑常驻 `BuffEntityAttributeSingleTarget`（现役无配置，机制留存）。**max_count 次数门控只在候选层生效，与 BUFF 类型/叠加逻辑完全独立、互不影响**。
...
- **监听处**：
  ...
  - `GameFightLogic.EventForAbyssalBlessingChange` → ① 立即对防守核心 + 全部普通防守生物 `RefreshBaseAttribute`，使属性类馈赠（含单体定向翻倍）当场生效（不必等切BOSS关重载场景）；② 调 `RefreshAbyssalBlessingOrbit` 刷新魔王身边环绕的馈赠图标（GPU单Mesh，全量对账增删——升级替换先删后加天然兼容；实现见 FightCreatureEntityForDefenseCore.cs 同名 region + `Resources/Shaders/Shader_AbyssalBlessingOrbit.shader`）。
...
ui.SetDataForTest(new long[] { 1000001001, 2000001001 },
...
> ⚠️ **时序铁律**：馈赠 BUFF 目标/施加者都是防守核心 UUID，**必须在防守核心创建之后调用**（`GameFightLogic.PreGame` 创建核心后，或战斗进行中）。
> 此前调用（如战斗场景未启动时）会 `LogWarning` 跳过、不添加（历史 BUG：GameTestEditor.GetTestData 在启动战斗前直接调导致 NullReferenceException，已改为走 `GameFightLogicTest.PreGameForAfterCreateDefenseCore` 钩子统一添加，详见 test-system SKILL「战斗场景测试」）。
> 按"族根+等级"取目标行配置用 `AbyssalBlessingInfoCfg.GetItemDataByFamilyLevel(familyRootId, level)`（遍历全表带缓存，仅供测试等低频场景）。
...
**卡片音效**：出现 `AnimForShow` 每张卡错位延迟结束、动画启动时各播 `sound_card_8`（序列 `SetTarget(rt)` 可被 `transform.DOKill()` 整体杀掉，防刷新/选中后残留发声）；悬停 `OnPointerEnter` 播 `sound_card_7`（`isSelecting` 时不播）；选中 `UIFightAbyssalBlessing.OnClickForSelect` 接受点击时播 `sound_card_5`（同生物卡片出现音效模式，见 creature-card-system）。
...
| 再来一瓶 | 1000003001 | 3001000001 | `BuffEntityInstantRewardMoreSelect` | `FightBeanForConquer.rewardAddSelectNum` | `RewardSelectBean.selectNumMax += n`，随后裁剪到不超过 `listReward.Count - 1`（首箱保底自动开启不占选择次数；超出的次数无对应宝箱可开，自然失效） |

> 两个馈赠图标当前均用 `ui_abyssalblessing_10`。新增同类「领奖参数型」馈赠时沿用此模式：写一个 `BuffEntityInstant` 子类累加到 `FightBeanForConquer` 计数器 → 在领奖初始化处读取应用；切勿用 BUFF 容器查询累计（即时 BUFF 触发即弃、容器内实例不承载累计数值，无法据此叠加）。详见 `fight-reward-system` skill 的领奖生成链路。

## 单体定向馈赠（随机一只防守生物属性/攻速翻倍）

> ⚠️ **现役无配置**：原 4 个单体定向馈赠已于 2026-07 从配置表删除（含其 BUFF 与多语言条目），**C# 基础设施完整保留**，新增同类馈赠按下述机制直接配置即可。

「可重复·限次(`level=0` + `max_count`)」单体定向馈赠的效果**只作用于随机一只防守生物**（不是全体、也不是防守核心）。推荐写法：`level=0` + `max_count=N`（`1`=整局限 1 次；改 `max_count` 即可调一局可获次数、与 BUFF 逻辑完全独立），BUFF 绑常驻单体定向型（非 Instant）；`level<=0` 故 UI 不显示等级角标。
...
- **会显示**（机制）：全体防守属性/攻速加成（每张防守卡各一份）+ 定向到本魔物的单体定向馈赠（按锁定 UUID 精确匹配）。⚠️ 现役 4 族馈赠（增殖/奖励多多/再来一瓶/钱多多）均不改生物数值，**当前卡面不会出现馈赠图标**；历史会显示的例：强身健体/大力出奇迹等（2026-07 已删）。
- **不显示**：作用敌方进攻生物的(`trigger=FightAttack`，历史例：慢条斯理)、只作用防守核心的、掉落类(钱多多 `BuffEntityConditional`)、奖励类(奖励多多/再来一瓶 `BuffEntityInstant`)、复制类(增殖 `BuffEntityInstant`)——它们不改本魔物数值。
...
> ⚠️ **馈赠侧现役无配置**：原 6 族动态率馈赠（都是兄弟·攻击/护甲/生命、杀红了眼·攻击/护甲/生命，每族 5 级共 30 行）已于 2026-07 删除，**C# 基础设施完整保留**（含广播重算链路）。当前 `HasDynamicRateAbyssalBlessing()`（馈赠侧）恒为 false；但广播落点守卫已扩展为双路 Or（`|| HasDynamicRateCreatureBuff()` 生物侧，2026-08 起 R 稀有度BUFF「独行者」走此路现役生效），新增同类馈赠按下述机制配置即可。

又一类馈赠的加成率**不是配置写死的固定值**，而是随运行时战况（场上魔物数量 / 单只自身累计击杀数）**每次重算属性时动态计算**。它们的 BUFF 继承抽象基类 `BuffEntityAttributeDynamicRate : BuffEntityAttribute`（重写 `CollectModifiers`+`ChangeData` 用 `GetDynamicRate()` 替代配置固定率，仅走 PercentAdd，用于 ATK/DR/HP；详见 `buff-system` SKILL「动态率深渊馈赠」）。作用范围两类：**全体防守魔物**（`trigger_creature_type=1`，不含核心）或**随机锁定的单只防守魔物**（兼实现 `IBuffSingleTarget`，`trigger_creature_type=1` 仅作防守类兜底过滤）。

| class_entity（通用功能类，可复用） | 缩放来源 | 公式（rate=每次重算属性时算） | 作用范围 | 曾用于 |
|------|--------------|------|------|------|
| `BuffEntityAttributeScaleByDefenseCount`（data=ATK/DR/HP） | 场上存活防守魔物数 | `(N - 1) × trigger_value_rate`；N 数 `dlDefenseCreatureEntity.List` 中 `!IsDead()`，N≤1 为 0（减 1 扣自身） | 全体防守 | 都是兄弟（已删） |
| `BuffEntityAttributeScaleByKillCount`（data=ATK/DR/HP，兼 `IBuffSingleTarget`） | 锁定魔物自身累计击杀数 | `GetRecordsForCreatureData(锁定UUID,false)?.killNum × trigger_value_rate`（仅魔物击杀；killNum 按 `creatureUUId` 持久累积，该只阵亡后下一关重上场 UUID 不变、加成保留，征服 run 内跨关卡累积至 BOSS 关不重置） | 单体定向 | 杀红了眼（已删） |

历史 id 段参考（2026-07 已删，段已释放）：都是兄弟 馈赠 `2000008001~2000010005` / BUFF `3001500001~3001700005`（图标 120~122）；杀红了眼 馈赠 `2000011001~2000013005` / BUFF `3001800001~3002000005`（图标 123~125）。

- 共性写法：多级升级链 `level=1~5`、`parent_id` 链式、`trigger_creature_type=1`、`valid=1`、`max_count=0`，每级 `trigger_value_rate=0.01~0.05`（每只/每杀 +1%~5%）。BUFF 的 `name`/`content` 指向本族根 BUFF id、语言用 `{Percentage}` 占位；馈赠语言逐级写死数值。
- **当前 id 进度**（2026-07 精简后，2026-07 新增第六次冲击）：现役深渊馈赠 BUFF 段（`buff_type=3`）→ `3000100001`、`3000200001~005`、`3000300001~005`（闪电）、`3000400001~005`（失控的矿车）、`3000500001~005`（死亡回旋）、`3000600001~005`（跳跳斧）、`3000700001~005`（知识的力量）、`3000800001~005`（回弹菱块）、`3000900001`、`3001000001`、`3001100001~005`（第六次冲击）、`3001200001~005`（瓶装炼狱火）；现役馈赠多级族号 1（钱多多）、2（闪电）、3（失控的矿车）、4（死亡回旋）、5（跳跳斧）、6（知识的力量）、7（回弹菱块）、8（第六次冲击）、9（瓶装炼狱火），单行族号 1~3；BUFF 族号 9、13~20 与馈赠多级族号 10~13、单行族号 4~7 **已释放可复用**（也可避开历史段从新号续编）。彩色图标编号历史用到 125，其中 `ui_abyssalblessing_120~125` 等闲置未删。

**广播重算（关键，动态率靠它生效，事件驱动）**：rate 变化后必须重算 `dicAttribute` 才生效。为此在高频事件按守卫广播全体防守魔物重算：

- 守卫（**热路径 O(1)，勿在死亡等高频点遍历池**）：动态率场景用 **`BuffHandler.Instance.HasDynamicRateAbyssalBlessing()`**——读 `BuffManager.hasDynamicRateAbyssalBlessing` 缓存布尔，仅当本局带了动态馈赠才广播，避免普通对局无谓开销。该缓存**单调置位**：`BuffHandler.AddAbyssalBlessing` 选取到含动态率BUFF的馈赠时直接置 `true`（一整局只增不减——馈赠不会中途删除，升级替换同族仍带动态BUFF），仅 `BuffManager.ClearAbyssalBlessing`（全通关领奖后）复位为 false。故**无需**重新遍历整池、也无独立 Refresh 方法。**2026-08 起扩展为双路 Or**：另一路 `BuffHandler.HasDynamicRateCreatureBuff()`（读 `BuffManager.hasDynamicRateCreatureBuff`）覆盖**生物侧动态率BUFF**（如 R 稀有度「独行者」）——`AddFightCreatureBuff` 内三处创建点收口为 `CreateFightCreatureBuffEntity`，创建实例 `is BuffEntityAttributeDynamicRate` 即单调置位，`BuffManager.ClearFightCreatureBuff`（战斗清理）复位。
...
- 两处广播落点：① `GameFightLogic.EventForGameFightLogicCreatureDeadEnd` 中——魔物死亡（随魔物数缩放类 N 减少）/敌人死亡（随击杀数缩放类计数增加）都重算，且**该重算放在 `CheckGameEnd()` 之前**（先处理死亡带来的属性变化，再检测游戏结束）；② `CreatureHandler.CreateDefenseCreatureEntity` 末尾（加完生物 BUFF 后）**推送新事件** `EventsInfo.GameFightLogic_DefenseCreatureCreate`（参数 FightCreatureEntity），由 `GameFightLogic.EventForDefenseCreatureCreate` 监听→按守卫重算，使随魔物数缩放类馈赠随 N 增大即时生效（CreatureHandler 只负责生成、推事件，重算职责归 GameFightLogic）。二者均先过 `HasDynamicRateAbyssalBlessing() || HasDynamicRateCreatureBuff()` 双路守卫（后者覆盖生物侧动态率BUFF，如独行者）。
...
| 馈赠配置扩展（IsLevelUp / GetFamilyRootId / GetFamilyMaxLevel / GetItemDataByFamilyLevel） | `Assets/Scripts/Bean/MVC/Game/AbyssalBlessingInfoBeanPartial.cs` |
...
| 馈赠变化刷新属性 | `Assets/Scripts/Game/Logic/GameFightLogic.cs`（EventForAbyssalBlessingChange：监听 Buff_AbyssalBlessingChange → 刷新防守核心 + 全部防守生物 RefreshBaseAttribute + 刷新魔王环绕图标 RefreshAbyssalBlessingOrbit） |
| 魔王环绕馈赠图标 | `Assets/Scripts/Game/Fight/FightCreatureEntityForDefenseCore.cs`（「魔王-深渊馈赠环绕图标(GPU单Mesh)」region：预制 AbyssalBlessingOrbit 节点 + 单Mesh + URP shader，公转/浮动/入场缩放全在vertex shader按_Time.y匀速；shader为框架层 `Assets/FrameWork/Shader/URP/Shader_Mesh_Orbit.shader`（FrameWork/URP/MeshOrbit）） |
...
| 环绕书本馈赠BUFF（知识的力量） | `Assets/Scripts/Game/Buff/BuffEntity/Periodic/BuffEntityPeriodicAttackOrbit.cs`（宿主选择/书本生成销毁与同步） + `Assets/Scripts/Game/Fight/AttackMode/AttackModeOrbit.cs`（常驻环绕弹道） |
| 周期地形火焰馈赠BUFF（瓶装炼狱火） | `Assets/Scripts/Game/Buff/BuffEntity/Periodic/BuffEntityPeriodicAttackFireBottle.cs`（周期触发→快照存活敌人不放回抽N目标→逐个投掷火瓶，连发间隔0.2s） |
| 单体定向馈赠BUFF（现役无配置，机制留存；曾用于大力出奇迹/膘肥体壮/钢铁憨憨/急性子） | `Assets/Scripts/Game/Buff/BuffEntity/Attribute/BuffEntityAttributeSingleTarget.cs` / `BuffEntityAttributeAttackTimeSingleTarget.cs` / `Assets/Scripts/Game/Buff/Interface/IBuffSingleTarget.cs`(接口，仅 SingleTargetCreatureUUId；不限深渊馈赠) |
| 动态率BUFF（馈赠侧现役无配置、机制留存，曾用于都是兄弟/杀红了眼；稀有度BUFF侧现役「独行者A-D」11001300001~04） | `Assets/Scripts/Game/Buff/BuffEntity/Attribute/BuffEntityAttributeDynamicRate.cs`(抽象基类，GetDynamicRate 动态率) / `BuffEntityAttributeScaleByDefenseCount.cs`(全体，"随场上魔物数缩放") / `BuffEntityAttributeScaleByKillCount.cs`(单体定向兼 `IBuffSingleTarget`，随机锁定一只，"随该只自身累计击杀数缩放") / `BuffEntityAttributeDecayByAllyCount.cs`(随场上魔物数反向衰减，独行者) |
| 动态率广播重算 | `Assets/Scripts/Component/Handler/BuffHandler.cs`(双路 O(1) 缓存守卫：`HasDynamicRateAbyssalBlessing()` 读 `BuffManager.hasDynamicRateAbyssalBlessing`（`AddAbyssalBlessing` 置位、`ClearAbyssalBlessing` 复位）+ `HasDynamicRateCreatureBuff()` 读 `hasDynamicRateCreatureBuff`（`AddFightCreatureBuff` 经 `CreateFightCreatureBuffEntity` 置位、`ClearFightCreatureBuff` 复位）) / `Assets/Scripts/Game/Logic/GameFightLogic.cs`(`RefreshAllDefenseCreatureAttribute()` 全体重算 + 死亡事件在 CheckGameEnd 前按双路守卫广播 + `EventForDefenseCreatureCreate` 监听放置事件按双路守卫广播) / `Assets/Scripts/Component/Handler/CreatureHandler.cs`(CreateDefenseCreatureEntity 末尾只推送 `GameFightLogic_DefenseCreatureCreate` 事件，不直接重算) |
...
10. **程序化添加后馈赠没生效（有告警日志）** → `AddAbyssalBlessing` 在防守核心创建前被调用（如战斗场景未启动），会 `LogWarning` 跳过；必须在 `GameFightLogic.PreGame` 创建核心之后（如 `PreGameForAfterCreateDefenseCore` 钩子）或战斗进行中调用。
