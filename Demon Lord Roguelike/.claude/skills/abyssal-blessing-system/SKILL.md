...
#### 三种馈赠类型（全部由 `level` + 族内行数决定，无需额外字段）

| 类型 | `level` | `parent_id` | 族内行数 | 候选行为 | 等级角标 |
|------|---------|-------------|----------|----------|----------|
| **单级可重复** | `0` | `0` | 1 | 始终可出现，重复选取叠加 | 不显示（`level<=0`） |
| **单级不可重复** | `1` | `0` | **仅 1 行**（无 lv2） | 选 1 次后不再出现 | 不显示（`IsSingleLevelOnce`） |
| **多级升级链** | `1..N` | 链式 | N（≥2） | 每次只出现"已拥有族的下一级" | 显示 `LvN` |

- **「单级不可重复」是「多级升级链」族内只有 1 级时的退化形态**：选取后 `GetAbyssalBlessingFamilyLevel` 返回 1，候选判定 `level==ownedLevel+1`(=2) 不成立 → 永不再出现，因此天然"不可重复"，**无需任何额外代码或字段**。
- 区分「单级不可重复」与「多级 lv1」靠 `AbyssalBlessingInfoCfg.IsSingleLevelOnce(info)`（= `level==1` 且 `GetFamilyMaxLevel(族根)==1`）；`GetFamilyMaxLevel` 返回族内最大 level（带缓存）。仅用于 UI 隐藏角标（[UIViewFightAbyssalBlessingItem.SetLevel](../../../Assets/Scripts/Component/UI/Game/FightAbyssalBlessing/UIViewFightAbyssalBlessingItem.cs)），不影响候选/BUFF 逻辑。
- BUFF 绑定：单级不可重复直接绑**常驻型** BUFF（属性/条件/周期），与多级各级一致；**不**像单级可重复那样需要 `BuffEntityInstant`+计数器（那是为"重复选取被同族替换移除后仍保留累加值"特意设计的，见下文"常驻可重复馈赠"）。
...
| 行号 | 含义 | 内容 |
|------|------|------|
| 第 1 行 | 字段名（导出 JSON 的 key） | `id` `icon_res` `parent_id` `level` `buff_ids` `name[language]` `details[language_1]` `remark` `valid` |
| 第 2 行 | 字段类型（导出工具识别） | `long` `string` `long` `int` `string` `long` `long` `string` `int` |
| 第 3 行 | 中文说明（仅给策划看） | `序号` `图标名字` `父级深渊馈赠ID` `等级` `buff_ids` `名字-中文` `描述中文` `备注` `是否有效(0无效1有效)` |
...
| `level` | int | ✓ | 等级，决定馈赠类型：`0`=单级可重复（不参与升级链）；`1` 且族内仅此 1 行=单级不可重复；`1..N` 链式=多级升级链。详见上文「三种馈赠类型」。 |
...
**单级可重复馈赠**（不参与升级、可重复选取叠加，如"增殖"）：1 行，`parent_id=0`、`level=0`，`valid=1`。

**单级不可重复馈赠**（选 1 次后不再出现、无等级、不显示角标）：1 行，`parent_id=0`、`level=1`，且**该族不要再加 lv2 行**，`valid=1`。BUFF 绑常驻型即可。
```
id=2000005001, parent_id=0, level=1, buff_ids=3000600001, name=2000005001, details=2000005001, remark=一次性强力馈赠（单级不可重复）, valid=1
```
