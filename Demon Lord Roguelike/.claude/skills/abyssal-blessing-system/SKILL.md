...
当前配置共 **18 行 / 6 族**（由 72 行 20 族精简而来，后新增闪电族、失控的矿车族）：
...
| 闪电（5 级链） | 2000002001~005 | 多级升级链（首个**周期型**馈赠） | 每 3 秒落下 1~5 道闪电随机击中敌人（0.1 秒间隔连发、不放回抽主目标——一轮内多道雷主目标互不重复，敌人少于雷数只发同等数量的雷；溅射不受限：同一目标可被多道雷重复溅射、被溅射过的目标仍可作主目标），落点 0.2~1 半径 AOE，单雷命中上限=等级（1~5 个，超上限按距落点近者优先），AOE 多目标伤害按命中次序依次减半（保底 1），伤害=魔王攻击力×BUFF `trigger_value` 倍率；BUFF `3000300001~005`=`BuffEntityPeriodicMultiInstantAttack`（`class_entity_data`="次数,攻击模块ID"，半径/单雷命中上限配在攻击模块表 300031~035 的 collider_area_size/hit_max），落雷粒子走全局单例 `Effect_Thunder_3`(id 900003，`EffectHandler.ShowThunderEffect`) |
| 失控的矿车（5 级链） | 2000003001~005 | 多级升级链（周期型道路冲撞） | 每 5 秒随机选 1~5 条道路，每条从最左端驶出 1 辆矿车沿路向右碾压（穿透整路、每个目标只撞一次），撞到的第 1 个敌人伤害=魔王攻击力×`trigger_value`，之后每多撞 1 个减半（保底 1），驶到路尽头消失；车数>路数时每路先各 1 辆、多出的随机重复分配，同路第 2 辆起间隔 0.5 秒错开出车；BUFF `3000400001~005`=`BuffEntityPeriodicRoadAttack`（`class_entity_data`="车数,攻击模块ID"），5 级共用攻击模块 300041=`AttackModeRangedPiercingRoad`（速度 10、collider_size 0.3、sound_hit=420001、残影拖尾），视觉 `AttackModeVisual_Minecart_1`，图标 `ui_abyssalblessing_minecar` |
...
> **已释放 id 段**：单行族 `1000004xxx~1000007xxx`、多级族号 4~13（`2000004xxx~2000013xxx`，族号 2 已被闪电占用、族号 3 已被失控的矿车占用）、BUFF 族号 5~8 与 11~20（`30005xxxxx~30008xxxxx`、`30011xxxxx~30020xxxxx`，族号 3 已被闪电占用、族号 4 已被失控的矿车占用）。闲置图标 `ui_abyssalblessing_1~5、8、11~14、120~125` 文件仍在图集中未删。
...
- **当前 id 进度**（2026-07 精简后）：现役深渊馈赠 BUFF 段（`buff_type=3`）→ `3000100001`、`3000200001~005`、`3000300001~005`（闪电）、`3000400001~005`（失控的矿车）、`3000900001`、`3001000001`；现役馈赠多级族号 1（钱多多）、2（闪电）、3（失控的矿车），单行族号 1~3；BUFF 族号 5~8、11~20 与馈赠多级族号 4~13、单行族号 4~7 **已释放可复用**（也可避开历史段从新号续编）。彩色图标编号历史用到 125，其中 `ui_abyssalblessing_120~125` 等闲置未删。
