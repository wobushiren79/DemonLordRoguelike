...
| 第六次冲击（5 级链） | 2000008001~005 | 多级升级链（周期型圆环冲击波；首个带**击退机制**的馈赠） | 每 10 秒从魔王位置+攻击起始偏移处发出一道圆环冲击波，以魔王为圆心扩张至覆盖整条道路（最大半径=道路右缘 0.5+路长 +余量 0.5 −圆心x，攻击模块按当场路长计算），扫到的敌人受魔王实时攻击力×`trigger_value`(Lv1~5=0.1~0.3)伤害（保底1、不暴击）并被**击退 0.5**（交 AI 击退意图 `AIIntentAttackCreatureKnockback`：方向固定 +x 沿道路向后推、不带 z 分量防推离路径、固定 0.2s 匀速推完、落点 x 钳制道路范围、攻击循环被打断、结束回闲置重新索敌），每只敌人每波只命中一次，场上无敌人本轮不触发；BUFF `3001100001~005`=`BuffEntityPeriodicAttackShockwave`（`class_entity_data`="攻击模块ID"），5 级共用攻击模块 300091=`AttackModeShockwaveRing`（XZ 环带命中自遍历不走射线批处理，speed_move=6=扩张速度，collider_area_size="余量,击退距离"="0.5,0.5"，sound_start=420001 占位待定），视觉 `Effect_Shockwave_1`(1700001) 走 `EffectHandler.ShowEnduringSingletonEffect`（全局单例通道，SingletonEffectParam 的 startSize/startLifetime multiplier 同步半径与扩张时长，视觉波前与判定环带严格重合），图标 `ui_abyssalblessing_boom` |
