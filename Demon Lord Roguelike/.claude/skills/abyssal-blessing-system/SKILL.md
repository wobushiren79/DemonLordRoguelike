...
| 跳跳斧（5 级链） | 2000005001~005 | 多级升级链（周期型弹跳斧头） | 每 5 秒从魔王处扔出 1~5 把斧头，抛物线追踪飞向**随机一排的最远敌人**（按 roadIndex 分组、同路 x 降序；不放回抽取——一轮内多斧首目标互不重复，敌人少于斧数只扔同等数量；第 1 斧立即+后续 0.2 秒间隔连发），伤害=魔王攻击力×`trigger_value`、逐目标减半保底 1；2 级起命中后弹跳至附近 1 单位内另一目标（Lv2~5 弹跳 1~4 次，同一斧头目标不重复，无候选即停），弹跳也是抛物线分段（弧高=首段一半，每段仅下落阶段命中）；BUFF `3000600001~005`=`BuffEntityPeriodicAttackBounceAxe`（`class_entity_data`="斧数,攻击模块ID,弹跳次数"），5 级共用攻击模块 300061=`AttackModeRangedArcBounce`（速度 4、collider_size 0.3、弹跳搜索半径配在 collider_area_size 第 1 项=1、sound_start=400003(发射音效 sound_fight_3)、sound_hit=420001、残影拖尾、弹体自旋 -720°/s、单段最短飞行时长 1s），视觉 `AttackModeVisual_Axe_1`，图标 `ui_abyssalblessing_axe` |
| 知识的力量（5 级链） | 2000006001~005 | 多级升级链（首个**常驻环绕型**馈赠） | 随机选取 1 只最前排（x 最大）的己方存活魔物作为宿主，其周围 XZ 水平面环绕 1~5 本书（半径 0.75、高度取魔王位置+攻击起始偏移、转速 Lv1~5=0.6/1.2/1.8/2.4/3 弧度/秒、角度均分），触碰到的敌人受到魔王实时攻击力×`trigger_value`(0.5) 的伤害、不暴击（每只书对同一敌人 0.5 秒冷却）；宿主死亡或出现更前排魔物（x 更大）→ 自动改选当前最前排（并列 x 随机 1 只、并列不换防抖动），场上无己方魔物 → 书本全销毁、重新有魔物再生成；书本=常驻攻击模块 `AttackModeOrbit`（不自毁、不做周期触发，`trigger_time=9999`），关卡切换被回收后下一帧整套重建；BUFF `3000700001~005`=`BuffEntityPeriodicAttackOrbit`（`class_entity_data`="书本数,攻击模块ID,转速"），5 级共用攻击模块 300071=`AttackModeOrbit`（attack_search_type=11 球形接触、collider_size 0.3、sound_hit=420001、残影拖尾），视觉 `AttackModeVisual_Book_1`（复用图标贴图），图标 `ui_abyssalblessing_book` |
| 回弹菱块（5 级链） | 2000007001~005 | 多级升级链（常驻反弹弹球型馈赠） | 场上**永久维持** 1~5 颗菱块（不随时间销毁、不随轮次累积、总数恒定=等级），菱块在道路矩形（x∈[0.5,0.5+路长]、z∈[0.5,路数+0.5]）内四壁反复反弹（越墙钳位+方向分量取反+绕 Y 轴 **±5° 随机偏转**防 90° 死角死循环；左墙在子弹进入场地后才生效防出生即弹回魔王侧），初始角度为以 +x 为 0° 的前向锥 ±75° 随机（保证射入道路范围），场上无敌人也照常发射；击中敌人伤害=魔王实时攻击力×`trigger_value`(0.5)、不暴击、不递减，**同一目标 1 秒冷却**（每颗弹独立记录各目标最近命中时刻）；弹速 Lv1~5=2/2.5/3/3.5/4 配在攻击模块行 `speed_move`；BUFF `3000800001~005`=`BuffEntityPeriodicAttackRebound`（`trigger_time=1` 每秒自检补弹到 N 颗——关卡切换存量弹被 `ClearAttackModePrefab` 清掉后自动补回，升级替换/清空时 `ClearData` 主动销毁全部存量弹；`class_entity_data`="弹数,攻击模块ID"），攻击模块 300081~300085=`AttackModeRangedRebound`（collider_size 0.6、sound_hit=420001、残影拖尾、弹体不自旋），视觉 `AttackModeVisual_Pingpang_1`（贴图 `Textures/Game/Pingpang_1.png` 复用图标），图标 `ui_abyssalblessing_pingpang` |
| 第六次冲击（5 级链） | 2000008001~005 | 多级升级链（周期型圆环冲击波；首个带**击退机制**的馈赠） | 每 10 秒从魔王位置+攻击起始偏移处发出一道圆环冲击波，以魔王为圆心扩张至覆盖整条道路（最大半径=道路右缘 0.5+路长 +余量 0.5 −圆心x，攻击模块按当场路长计算），扫到的敌人受魔王实时攻击力×`trigger_value`(Lv1~5=0.1~0.3)伤害（保底1、不暴击）并被**击退 0.5**（交 AI 击退意图 `AIIntentAttackCreatureKnockback`：方向固定 +x 沿道路向后推、不带 z 分量防推离路径、固定 0.2s 匀速推完、落点 x 钳制道路范围、攻击循环被打断、结束回闲置重新索敌），每只敌人每波只命中一次，场上无敌人本轮不触发；BUFF `3001100001~005`=`BuffEntityPeriodicAttackShockwave`（`class_entity_data`="攻击模块ID"），5 级共用攻击模块 300091=`AttackModeShockwaveRing`（XZ 环带命中自遍历不走射线批处理，speed_move=6=扩张速度，collider_area_size="余量,击退距离"="0.5,0.5"，sound_start=420001 占位待定），视觉 `Effect_Shockwave_1`(1700001) 走 `EffectHandler.ShowShockwaveEffect`（startSize/startLifetime multiplier 同步半径与扩张时长，视觉波前与判定环带严格重合），图标 `ui_abyssalblessing_boom` |
| 瓶装炼狱火（5 级链） | 2000009001~005 | 多级升级链（周期型抛物线地形火焰） | 每 10 秒向不放回抽取的 1~5 个敌人投掷炼狱火瓶（第1瓶立即+后续 0.2 秒间隔连发；敌人少于瓶数只丢同等数量——同轮多瓶不瞄同一目标；场上无存活敌人本轮不触发），瓶子抛物线飞向该目标当前位置（**固定落点、不追踪**，目标中途死亡/移动照飞原落点），落地燃放半径 1.2 地形火焰持续 5 秒，每 1 秒对火焰上敌人造成魔王实时攻击力×`trigger_value`(0.1)伤害（保底1、不暴击、不递减——多片叠加可多次跳伤）；BUFF `3001200001~005`=`BuffEntityPeriodicAttackFireBottle`（`class_entity_data`="瓶数,攻击模块ID"），5 级共用攻击模块 300101=`AttackModeRangedArcGround`（sound_start=400003(发射音效 sound_fight_3)；双状态：飞行段抛物线不追踪+纯投掷物禁用命中+弹体自旋-720°/s绕-Z轴，燃烧段每1秒 CheckHitTargetArea 半径1.2跳伤、满5秒自毁；collider_area_size 第1项=燃烧半径；飞行段 DSP 火瓶贴图 `AttackModeVisual_FireBottle_1`(贴图复用馈赠图标)，燃烧段 visualBucketKey 置空隐藏 DSP 改走 `EffectHandler.ShowFloorFireEffect` 全局单例 burst 重播粒子 `Effect_FloorFire_1`(1800001)，粒子时长同步燃烧5秒），图标 `ui_abyssalblessing_fire` |
...
战斗中点开魔物卡牌的详情 popup(`UIViewCreatureCardDetails`)展示 HP/DR/ATK/ASPD 时，必须用 **`CreatureBean.GetAttribute(类型, includeAbyssalBlessing: true)`** 才会反映深渊馈赠（含单体定向翻倍）；漏传 `true` 会导致详情面板数值与场上实际值不符（历史典型症状：单体定向翻倍馈赠生效后场上伤害翻倍但详情攻击力没变）。
...
> ⚠️ **现役无配置**：原 6 族动态率馈赠（都是兄弟·攻击/护甲/生命、杀红了眼·攻击/护甲/生命，每族 5 级共 30 行）已于 2026-07 删除，**C# 基础设施完整保留**（含广播重算链路）。当前 `HasDynamicRateAbyssalBlessing()` 恒为 false、广播不会触发；新增同类馈赠按下述机制配置即可。
...
| 动态率馈赠BUFF（现役无配置，机制留存；曾用于都是兄弟/杀红了眼） | `Assets/Scripts/Game/Buff/BuffEntity/Attribute/BuffEntityAttributeDynamicRate.cs`(抽象基类，GetDynamicRate 动态率) / `BuffEntityAttributeScaleByDefenseCount.cs`(全体，"随场上魔物数缩放") / `BuffEntityAttributeScaleByKillCount.cs`(单体定向兼 `IBuffSingleTarget`，随机锁定一只，"随该只自身累计击杀数缩放") |
