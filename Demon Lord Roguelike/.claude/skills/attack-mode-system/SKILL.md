...
├── AttackModeSummon                - 召唤（瞬发无弹道无伤害：当帧按 other_data 键 summon_npc_ids[逗号分隔NPC池,每只独立随机]&summon_count[每路数量,默认1]&road_spread[上下扩展路数,默认0=仅自己路,越界路自然衰减跳过不clamp] 经 CreatureHandler.CreateAttackCreature 在攻击者当前位置[x±0.25抖动]生成进攻生物[强度倍率透传 attacker.intensityRate 随关卡递增]，召唤物即置 FightCreatureBean.isSummoned[当前消费=死亡不掉魔晶,防低血召唤物被挂机刷取]+发起出土冒出[通用意图 AIIntentCreatureEmerge 从地底冒出,见 ai-system]；当帧完成无实例状态对象池天然安全；骷髅召唤师：普攻 500004[脚下召1只骷髅战士,生物103105 attack_pre_time=4.5+anim_attack_time=0.5=5秒一次] / BOSS技能 500005[自己路+上下相邻路每路3只共最多9只,每只独立随机骷髅战士/投手]经 ext 100008[trigger_scene=0,trigger_interval=30]攻击意图内30秒一发顶替当次普攻[大魔法师BOSS同款]；召唤物NPC 20010001/20020001 引用克隆生物 103106/103107[克隆2001/2002仅改 attack_search_creature_fight_type=1 防作为进攻方打友军+creature_type=2,NPC行HP=20]；解析 GetSummonConfig()，string 键走 ParseOtherDataString[2026-09-15 新增]）
