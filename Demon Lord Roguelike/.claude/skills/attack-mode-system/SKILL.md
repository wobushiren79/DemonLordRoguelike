...
├── AttackModeShockwaveRing         - 圆环冲击波（以魔王为圆心扩张：XZ 环带命中自遍历不走射线批处理、每敌每波只中一次、命中瞬时击退、最大半径=道路全长+余量达到即销毁；视觉走 EffectHandler.ShowEnduringSingletonEffect 全局单例通道，特效id取攻击模块配置 effect_hit=1700001(Effect_Shockwave_1,原 manager effectShockwaveId 常量已删)，用 SingletonEffectParam 的 multiplier 字段同步半径/时长；深渊馈赠「第六次冲击」300091）
