...
├── AttackModeShockwaveRing         - 圆环冲击波（以魔王为圆心扩张：XZ 环带命中自遍历不走射线批处理、每敌每波只中一次、命中瞬时击退、最大半径=道路全长+余量达到即销毁；视觉走 EffectHandler.ShowEnduringSingletonEffect 全局单例通道，特效id取攻击模块配置 effect_hit=1700001(Effect_Shockwave_1,原 manager effectShockwaveId 常量已删)，用 SingletonEffectParam 的 multiplier 字段同步半径/时长，并经 actionForGet 回调缓存实例、Update 逐帧对齐 simulationSpeed=GetCurrentGameSpeed 使视觉扩张跟随游戏速度；深渊馈赠「第六次冲击」300091）
...
├── AttackModeShieldCast            - 援护护盾施法（瞬发无弹道无伤害：当帧搜索同阵营最前排 N 个存活友军[排除自己，走 FightCreatureSearchUtil.FindFrontRowCreatures]逐个套 other_data 键 shield_buff 指定的护盾BUFF[applier=自己]；出手特效 effect_hit 播在自己出手点[自身位置+攻击模块 start_pos_offset 偏移，空=0,0,0 播在脚下；不用生物 attack_start_position，避免牵连普攻命中特效取点]，出手音走 sound_start 由 GetAttackModePrefab 自动播；大盾战士BOSS技能 500003[other_data=shield_count:3&shield_buff:2000700001，effect_hit=500003=Effect_Buff_Def_1，sound_start=470001]，经 AttackModeExtInfo 100005[trigger_scene=1 释放技能意图]挂载——NPC ai_param=skill_update:100005:10 注册通用 Update 事件固定 10s 触发「释放技能」意图发射，走路也放[2026-09-14 定稿]）
├── AttackModeSummon                - 召唤（瞬发无弹道无伤害：当帧按 other_data 键 summon_npc_ids[逗号分隔NPC池,每只独立随机]&summon_count[每路数量,默认1]&road_spread[上下扩展路数,默认0=仅自己路,越界路自然衰减跳过不clamp]&summon_offset_x[召唤落点相对攻击者的x偏移,默认0] 经 CreatureHandler.CreateAttackCreature 在攻击者前方生成进攻生物[落点x=攻击者x+summon_offset_x,世界轴原始叠加照 Melee 约定,y/z由生成管线决定；强度倍率透传 attacker.intensityRate 随关卡递增]，召唤物即置 FightCreatureBean.isSummoned[当前消费=死亡不掉魔晶,防低血召唤物被挂机刷取]+发起出土冒出[通用意图 AIIntentCreatureEmerge 从地底冒出,不播出场特效,见 ai-system]；当帧完成无实例状态对象池天然安全；骷髅召唤师：普攻 500004[前方0.5召1只骷髅战士,生物103105 attack_pre_time=4.5+anim_attack_time=0.5=5秒一次] / BOSS技能 500005[前方0.5,自己路+上下相邻路每路3只共最多9只,每只独立随机骷髅战士/投手]经 ext 100008[trigger_scene=0,trigger_interval=30]攻击意图内30秒一发顶替当次普攻[大魔法师BOSS同款]；两行均配 summon_offset_x:-0.5=前方0.5[向左攻击者前方=X负值]+effect_hit=1000001(Effect_Mana_1 魔王耗蓝同款出手特效,播在攻击者脚下=start_pos_offset 空;召唤落点与出手特效取点两组独立配置互不影响)；召唤物NPC 20010001/20020001 引用克隆生物 103106/103107[克隆2001/2002仅改 attack_search_creature_fight_type=1 防作为进攻方打友军+creature_type=2,NPC行HP=20,随机外貌=skin_data空+creature_random_id=2000001骷髅通用池]；解析 GetSummonConfig()，string 键走 ParseOtherDataString[2026-09-15 新增]）
├── AttackModeInstant               - 单体瞬时攻击（无弹道飞行，当帧对 AI 锁定单体 UnderAttack 结算；命中特效播在 目标位置+攻击者 attack_start_position 偏移[劈在目标躯干，照 FalluponChain.ExecuteAttack 取点先例]，当帧回收+回调；与 InstantArea 范围瞬击对称；牧师/神官普攻 102003[effect_hit=2100001(Effect_Hit_2)，sound_hit=420008]，生物 103108 attack_pre_time=0.5+anim_attack_time=0.5≈1秒一次[2026-09-16 新增]）
...
| `AttackModeMeleeMulti` | 瞬间命中同一目标 N 段（段间隔可配 hit_times&hit_interval，首段当帧+立即回调，余段 Update 按 GetFightDeltaTime 追加，目标死亡放弃余段） | 多段近战普攻（盗贼 100002） |
...
| `AttackModeRegainHPArea` | 群体治疗-自身周围（当帧遍历同阵营存活友军[含自己]，筛 距离≤collider_area_size[0] 且 HP不满 逐个 RegainHP+各播一次性治疗粒子[ShowEffect 独立实例]） | 自身为中心群体治疗（牧师/神官技能 500006，ext 100009 全局3秒） |
| `AttackModeRegainHPGround` | 地面持续治疗区域（施法快照圆心/阵营/半径，当帧即回调，区域 Update 自持每秒跳 RegainHP、满5秒自毁；治疗量=ATK×damage_add_rate 快照；区域视觉走 ShowEffect 独立实例） | 地面持续治疗圈（神官BOSS技能 500007，ext 100010 全局10秒） |
| `BaseAttackMode` | 完全自定义 | 特殊机制攻击 |

### 3. 创建攻击模式类

> **时间推进约定（2倍速）**：弹道/攻击模式内一切按时间推进的逻辑（飞行、重力、进度累加）必须用 `GameFightLogic.GetFightDeltaTime()`（= `Time.deltaTime × 当前游戏速度`，非战斗场景恒 1 倍）**替代 `Time.deltaTime`**，否则 2倍速下该弹道仍是 1 倍节奏。现有 `AttackModeRanged/Tracking/Arc/Fallupon(Area)/SplitChild/Chain` 已全部遵守。
...
冲锋自爆型防守生物（`CreatureInfo.charge_attack=1`，如 6003 哥布林敢死队）的爆炸**不走** `AttackModeExplosion.StartAttack(attacker,…)`——该路径对已死攻击者直接销毁不爆。统一自爆路径：

1. **三条引爆触发汇聚同一入口**：冲锋遇敌（冲锋意图索敌命中）/ 冲到路尽头 / 被打死（CheckDead）→ 均调 `FightCreatureEntity.SetCreatureDead()`（新增 `if (IsDead()) return;` 幂等守卫，保证只爆一次）。
2. 死亡分发 `SetCreatureDeadForDefense()`（FightCreatureEntityForDefense.cs）里 `IsChargeAttack()` 为真则调 **`CreateAttackModeForDeadExplosion()`**。
...
// AttackModeInfoBean 关键配置字段
{
    ...
    "other_data": "stuck_time:3&stuck_sink:0.1", // 其它扩展参数（&分隔项,:分键值，按 key 解析的通用扩展列）。当前键：stuck_time=射空后插地停留秒数(>0启用插地)、stuck_sink=插地下沉深度(默认0)，插地目前仅 AttackModeRangedArcTracking 系使用，解析见 GetStuckConfig()；aim_up=瞄准点相对目标脚底上抬高度(默认0=瞄脚底)，目前仅 AttackModeRangedObliqueTracking 使用，解析见 GetAimUpHeight()；dr_damage_rate/hp_damage_rate=护甲/血量分段伤害倍率(默认1/1=不启用；非默认走串联破甲结算：护甲>0只以dr倍率打甲不掉血、破甲后只以hp倍率打血、破甲击溢出不结转，结算在 FightCreatureEntity.UnderAttack，经 FightUnderAttackBean 注入)，目前仅牛头人法师 101003/101004(dr2/hp0.5) 使用，解析见 GetDamageSplitConfig()；hit_decay=瞬时AOE多目标伤害递减率(默认0.5=依次减半，配1=全额)，目前仅 AttackModeInstantArea 使用，解析见 GetHitDecayRate()；gravity=天降弹道重力加速度(默认9.81，每帧累加下落速度，配小则更慢更飘)，目前仅火/水大魔法师BOSS技能 700001/700002(gravity:2.4525=9.81 两次减半) 使用，解析见 GetGravity()，消费在 AttackModeFallupon(Area) 的 currentFallSpeed 累加；shield_count/shield_buff=援护护盾施法的最前排友军数量(默认3)/护盾BUFF的ID(默认0=未配置空放)，目前仅 AttackModeShieldCast（大盾战士BOSS技能 500003）使用，解析见 GetShieldCastConfig()（long 键走 ParseOtherDataLong）；summon_npc_ids/summon_count/road_spread/summon_offset_x=召唤的NPC id池(逗号分隔,每只独立随机;空=未配置空放)/每路召唤数量(默认1)/上下扩展路数(默认0=仅自己路,越界自然衰减)/召唤落点相对攻击者的x偏移(默认0;世界轴原始叠加,向左攻击者前方=X负值)，目前仅 AttackModeSummon（骷髅召唤师普攻 500004/BOSS三路技能 500005）使用，解析见 GetSummonConfig()（string 键走 ParseOtherDataString，2026-09-15 新增）
...
| `GetVelocityClampSpeed()` | 渲染器瞬移钳制基准速度（帧差分速度超过它 ×1.5×gameSpeed 即判瞬移清零）；基类默认=GetMoveSpeed，加速弹道（天降 Fallupon/Area 重力加速）重写返回当前真实速度（currentFallSpeed），否则后半程每帧被误钳、DSP 火星甩尾消失 |
...
- **分桶 key = `attackModeInfo.visual_name`（新配置字段）**：`RegisterVisual(visualKey, mesh, material, visualConfig = default, spinAxis = default, spinSpeed = 0f)` 登记视觉桶（mesh=朝相机 Quad，material 须开 GPU Instancing；`visualConfig`=配置表 `visual_data` 解析的 per-bucket 渲染参数，见「视觉渲染参数 visual_data」；**自旋在此一次性写进桶材质**，见下方 per-instance 条目）；每桶固定 `Matrix4x4[1023]` 复用缓冲，满批即绘 + 收尾绘剩余，**无热路径分配**；绘制走 `DrawBucket`，弹体**默认不投/不收阴影**（`BodyShadowCasting`/`BodyReceiveShadows` 常量是桶的默认值 `Off`/`false`——满屏子弹的阴影几乎看不出却让每桶多走一遍 ShadowCaster Pass；现由 `visual_data` 的 `cast`/`receive` **逐桶控制**，3D 立体模型如矿车可开）；用 `position` 构建 `TRS`（旋转暂用单位、缩放取 `visualScale`（`<0`/未配置时取1、大小改由桶材质自身 `_VertexScale` 决定），billboard 交 shader）。已删除 `uniformScale` 统一缩放字段。
...
- **⚠️`type:2` 的配置只需 `type` + `color`（其余参数已写死在 EffectHandler）**：`count`/`interval`/`startAlpha`/`endAlpha` 本就是**桶级**(注册时灌进 VFX 实例、同 visualKey 只注册一次，逐行填了也只有首个 `type:2` 行生效)、`shrink` 方案2 不支持(仅方案1 的矩阵缩放衰减)，留在配置表里等于误导策划"可逐行调"，故段数/间隔/透明度已**从配置表移除**、统一收进 `EffectHandler` 的 `TrailVfxLifetime`(1s) / `TrailVfxSpawnInterval`(0.02s) / `TrailVfxStartAlpha`(0.5) / `TrailVfxEndAlpha`(0.05) / `TrailVfxParticleSize`(0.05，粒子尺寸；曾取弹体材质 `_VertexScale` 跟随弹体，现全局写死) 五个常量——**要调 VFX 拖尾表现就改这五个常量**(单发同时存活粒子数≈Lifetime/SpawnInterval≈50)。`type:2` 的行再填 count/interval/alpha/shrink **会被静默忽略**。
...
- **已知局限/待办**：朝相机 billboard（弹体本体）待 shader(Editor 里看到的是未朝相机的静态 Quad；火球 shader 例外——它自己在世界空间取相机右/上轴展开)；**拖尾方案2(VFX `type:2`) 已完整落地**（C# 绑定 + 加载走 EffectHandler(res_name `Effect_Trail_1`) + 图 `VFX_Trail_1.vfx` 位置定位与逐弹染色均已接完）；⚠️**`trail_data` 的 `color` 在方案1 下是桶级(同 `visual_name` 首个注册者赢)、方案2 下才逐弹生效**；⚠️**方案2 下 `count`/`interval`/`startAlpha`/`endAlpha` 仍是桶级**(注册时灌进 VFX 实例，同 `visual_name` 首个 `type:2` 行赢)、**`shrink` 不支持**(仅方案1 的矩阵缩放衰减)——只有 `color` 逐弹，需要不同段数/间隔/透明度须拆 `visual_name`。
...
| **魔法弹 shader**(火球/冰球通用：核心 + 四散火星在 vertex shader 内模拟，同一次 draw；逐实例 `_VelocityWS` 世界速度做火星世界化 + `_SeedOffset` 相位错峰 + `_InstanceScale` 逐实例缩放) | `Assets/FrameWork/Shader/URP/Shader_Mesh_MagicBallInstanced_1.shader` |
...
| 近战多段（盗贼 100002） | `Assets/Scripts/Game/Fight/AttackMode/AttackModeMeleeMulti.cs` |
...
| 单体瞬时攻击（牧师/神官普攻 102003 神圣瞬击，命中特效 Effect_Hit_2=2100001） | `Assets/Scripts/Game/Fight/AttackMode/AttackModeInstant.cs` |
...
| 群体治疗-自身周围（牧师/神官技能 500006，ext 100009 trigger_scene=1 3秒） | `Assets/Scripts/Game/Fight/AttackMode/AttackModeRegainHPArea.cs` |
| 地面持续治疗区域（神官BOSS技能 500007，ext 100010 trigger_scene=1 10秒） | `Assets/Scripts/Game/Fight/AttackMode/AttackModeRegainHPGround.cs` |
| 援护护盾施法（大盾战士BOSS技能 500003，ext 100005 trigger_scene=1） | `Assets/Scripts/Game/Fight/AttackMode/AttackModeShieldCast.cs` |
| 召唤（骷髅召唤师普攻 500004 / BOSS三路技能 500005，ext 100008 trigger_scene=0 30秒） | `Assets/Scripts/Game/Fight/AttackMode/AttackModeSummon.cs` |
