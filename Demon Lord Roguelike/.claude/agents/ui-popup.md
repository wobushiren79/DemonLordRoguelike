...
- **研究门控**：线路数/关卡数/路径长度/奖励四项受「设施」研究门控，用 `userUnlock.CheckIsUnlock(UnlockEnum.PortalPreviewRoadNum / PortalPreviewFightNum / PortalPreviewRoadLength / PortalPreviewReward)` 判定，未解锁则该详情项整行隐藏（奖励区直接不显示）。**名字行、难度行始终显示**，不受门控。无尽模式（`GameFightTypeEnum.Infinite`）不展示难度/关卡数/路径长度/奖励（难度是征服模式专属概念）；挑战100勇士隐藏难度/关卡数两行。
