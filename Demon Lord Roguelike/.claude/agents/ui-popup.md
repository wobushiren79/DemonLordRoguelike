...
- **四个详情项**（[Component](Assets/Scripts/Component/UI/Popup/UIPopupPortalDetailsComponent.cs) 中 `ui_UIViewPopupProtalDetailsItem_Name/_RoadNum/_FightNum/_RoadLength`，类型均为 `UIViewPopupPortalDetailsItem`）：名字 / 线路数 / 关卡数 / 路径长度。每项调 `itemView.SetData(title, content, isShow)`；标题文本 id 依次为 411/412/413/414（路径长度 414 为本次新增项）。征服模式按 `gameWorldInfoRandom.GetDifficultyRandom(difficultyLevel)` 取该难度预生成的 roadNum/fightNum/roadLength；无尽模式直接用字段值。
...
- **研究门控**：线路数/关卡数/路径长度/奖励四项受「设施」研究门控，用 `userUnlock.CheckIsUnlock(UnlockEnum.PortalPreviewRoadNum / PortalPreviewFightNum / PortalPreviewRoadLength / PortalPreviewReward)` 判定，未解锁则该详情项整行隐藏（奖励区直接不显示）。**名字行始终显示**，不受门控。无尽模式（`GameFightTypeEnum.Infinite`）不展示关卡数/路径长度/奖励。
