...
- 气泡名字/难度始终显示(不门控)，线路数/关卡数/路径长度/奖励四项**未解锁整行隐藏**(非占位)；无尽模式不展示难度/关卡数/路径长度/奖励(难度为征服模式专属)；ChallengeHundred 隐藏难度/关卡数行(单关恒1)，奖励行全量预览(普通3箱/BOSS装备6箱；征服仅首箱保底 `listReward[0]`)。
