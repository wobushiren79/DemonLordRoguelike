...
- 异步任务：**GTask** - UniTask 门面（`Wait`/`WaitReal`/`WaitFrame`/`WaitFrames`/`WaitFrameEnd`/`WaitUntil`/`WaitWhile`/`WaitUntilTimeout`/`WaitTween` + `Run` 发射 + `WhenAll`/`WhenAny` 组合 + `GTaskCancel` 取消源（`NewCancel(gameObject)`/`NewCancelFor(Component)` 建源）；发射即忘方法声明 `async UniTaskVoid`、调用点 `_ = Method()` 显式丢弃，禁止 `async void`；详见 CLAUDE.md「异步与定时逻辑规则」与 utils-system skill）
