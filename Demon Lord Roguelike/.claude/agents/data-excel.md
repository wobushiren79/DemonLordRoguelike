---
name: data-excel
description: Excel配置表处理：ExcelUtil、EPPlus、ExcelEditorWindow配置导出、Excel-JSON转换。直接读写xlsx文件使用openpyxl脚本。包含游戏全部36张配置表。
...
- **ExcelEditorWindow** - Excel 编辑器窗口（导出 JSON，单文件导出 `ExcelToJsonItem(FileInfo)` 转调 ExcelUtil 静态实现）
  ...
  - **列头标记约定**：`[language]`/`[language_1]`/`[language_2]`（多语言 textId 指向，生成 `xxx_language` 属性）与 `[mode_id]`（Mod 配置行 id 指向，纯字段）——带这四种标记的字段在 `CreateEntity` 生成 Bean 时会**自动产出 `CombineModReferenceIds` 重写**（Mod JsonText 行合并时按同一 modId 拼接这些引用字段，0 不拼；int 列生成时 LogWarning 提示改 long），导出 JSON 侧（`ExcelUtil.ExcelToJsonItemForBase`）同步剥离标记。新增 Mod 引用列=列头加标记+重新生成该表 Entity，零代码（详见 editor-extension-system SKILL「Mod 引用列标记约定」、mod-system SKILL「合并机制」）
...
| `excel_creature_model_info[生物模型详情信息] .xlsx` | CreatureModelInfo | 440 | id, model_id, show_type, part_type, res_name, color_state |
...
| 文件名 | Sheet | 数据行 | 主要列 |
|--------|-------|--------|--------|
| `excel_attackmode_info[攻击方式].xlsx` | AttackModeInfo | 62 | id, class_name, prefab_name, visual_name(DSP批量渲染分桶key,空=不走DSP), buff, attack_search_type, collider_size/area, effect_hit/damage, speed_move, sound_start/miss/hit, start_pos_offset, trail_data(拖尾/残影:count残影数&interval采样间隔秒&startAlpha最新档透明度&endAlpha最老档透明度&shrink每档缩放递减步长(档k缩放=max(0,1-shrink*(k+1)),0默认=不缩放)&color染色rgb,需配visual_name,空=无拖尾), child_attack_mode_id, damage_add_rate(伤害加成比例,float,最终伤害=攻击者ATK×该值,0/空=按1倍;自爆史莱姆爆炸300001配50), hit_max(单次命中目标数上限,int,>0按距落点近者优先截断,0/空=不限;仅AttackModeInstantArea系), remark, visual_data(视觉渲染参数,per-bucket光照控制:&分隔项,:分键值; ambient环境光补偿方式 flat默认=6轴平均均匀/sh=方向性球谐(3D立体模型用), cast投阴影0/1默认0, receive收阴影0/1默认0; 空=全部默认; 矿车300041已配 ambient:sh&cast:1&receive:1) （22列）。100002=近战多段攻击(AttackModeMeleeMulti,复制100001改class+other_data配 hit_times:2&hit_interval:0.1,段间隔走GetFightDeltaTime不同帧,首段当帧回调余段静默追加,配给盗贼生物104001 attack_mode)；102001=BOSS技能"向前挥砍攻击前方6格1排"(AttackModeMeleeArea, type24, size"3,1,0.25", hit_max0不限, effect_hit400002, 持盾战士BOSS 1010020001经ext 100001挂载3s)；101006=大剑战士BOSS普攻(克隆101001战之魅魔: type24 size"0.5,1,1" hit_max3 effect_hit400003, 配给生物104101/104103长枪骑士 attack_mode)；101007=勇者专用普攻(2026-09-18新增,克隆101006改: size"1.5,1,1.25"[前方3格×上中下3排=3x3] hit_max0范围内全打, 配给勇者专用生物104104[NPC1041010003,独立于104101] attack_mode)；102002=BOSS技能"向前挥砍攻击前方6格上中下3排"(克隆102001改size"3,1,1.25"[路间距1,Z半宽1.25=本路+上下相邻路], 难度4大剑战士BOSS 1041010001经ext 100004挂载替换原100001)；300031~300035=落雷(AttackModeInstantAreaThunder,深渊馈赠闪电Lv1~5,半径0.2~1.0/hit_max1~5,sound_start=420003打击3每道雷发射播1次)；300041=矿车(AttackModeRangedPiercingRoad,深渊馈赠失控的矿车5级共用,沿路穿透碾压+伤害逐目标减半+到路尽头销毁,速度10,sound_start=310001(出车音效sound_clean_1,框架0.1s同音去重防同帧多车叠播),sound_hit=420001,visual_data=ambient:sh&cast:1&receive:1)；300081~300085=回弹菱块(AttackModeRangedRebound,深渊馈赠Lv1~5,道路矩形四壁永久反弹,speed_move2.0~4.0,sound_start=400003(发射音sound_fight_3,每颗发射播放,0.1s同音去重同帧多颗只播一声),sound_hit=420001,回弹音sound_hit_6为代码内HandleWallBounce播放非配置)；300002=爆炸范围攻击-火焰(敢死队)(AttackModeExplosion,复制300001改: collider_area_type=11/collider_area_size=1.5[爆炸伤害半径]/damage_add_rate=1/effect_hit=300002(Effect_Explosion_2,2026-08-18起爆炸两模块300001/300002均改用它,原Effect_Explosion_1已无引用)/sound_hit=420008/sound_miss=100001,配给哥布林敢死队6003冲锋自爆)；600001=引诱(AttackModeLure,改变被攻击者线路,effect_hit=500002粒子/sound_hit=470001)；500001=加血(AttackModeRegainHP,4002愈之魅魔,effect_hit=500001(Effect_Buff_Health_1)/sound_hit=470001)；500002=加护甲(AttackModeRegainDR,4005盾之魅魔,effect_hit=500003(Effect_Buff_Def_1,2026-08-22起由500002迁出独立,原500002=Effect_Buff_1归600001引诱独占)/sound_hit=470001)；205002/205003=穿透火锥/冰锥-逐击衰减收缩(2026-08-19 起 class_name 由 AttackModeRangedPiercing 改 AttackModeRangedPiercingShrink:初始2倍大小,每穿透1个敌人伤害减半保底1+大小缩小当前的1/3,伤害降至1的当次命中后销毁,buff=1000500002:0.1[10%烧伤]/1000100002:0.1[10%冰减速];兽人魔法师7003/7004用,7002兽人投手的205001仍为原 AttackModeRangedPiercing 不受影响)；101003/101004=牛头人法师5003火/5004冰 远程瞬时捶地地刺(2026-08-19 起 class_name 由 AttackModeMeleeArea 改 AttackModeInstantArea:无弹道当帧落点=目标脚下,hit_max5全额[other_data 配 hit_decay:1 覆盖默认0.5减半]+other_data 配 dr_damage_rate:2&hp_damage_rate:0.5 串联破甲[护甲>0双倍只打甲不掉血/破甲后半伤打血/溢出不结转]+buff=1000500002:0.1[10%烧伤]/1000100002:0.1[10%冰缓]同人类法师；生物表 5003/5004 的 attack_search_type 41→0、attack_search_range 6→10 同人类法师)；207001=远程攻击-攻击(人类弓手)(2026-08-22 复制200001改：sound_hit 420011→200008[sound_knife_hit_8]，配给人类-弓箭手102001/人类-强弓手102002/人类-连射手102003，原200001保留给骷髅投手2002等不受影响；同期 205001兽人投手/206001人类弓箭手/204101分裂弓箭子弹道 的 sound_hit 也统一改 200008)；700005=BOSS技能-冰大魔法师天降冰球(2026-09-11 新增,克隆700001火球改: buff=1000700001:0.5[50%冰缓]/effect_hit=200001[Effect_Boom_Ice_1];2026-09-13 visual_name 由 AttackModeVisual_RangedIceBall 小冰球改为新增大冰球视觉 AttackModeVisual_RangedIceBall_2,冰大魔法师BOSS 1031030001 经ext 100006挂载5s)；700006=BOSS技能-雷大魔法师三连雷击(2026-09-11 新增时为单次随机落点天降雷电[AttackModeFalluponAreaRandom,effect_hit=900003],2026-09-14 改造: class_name=AttackModeFalluponChainMulti 三连发发射器[自身无伤害无特效,首发当帧+Update按GetFightDeltaTime排程余发,攻击者死亡/实体池复用取消余发]+child_attack_mode_id=800002[BOSS版普攻雷击(2026-09-15 由 800001 拆出:复制普攻800001、初始+连锁特效均换 900004 Effect_Thunder_4,普攻800001仍用900001&900002不受影响),每发实体路径重新取ATK快照]+other_data=hit_times:3&hit_interval:0.25[0.5秒内3发,每发独立随机选全场存活防守生物不含核心],原 collider_area/effect_hit/speed_move/sound/start_pos_offset 已清空,雷大魔法师BOSS 1031040001 经ext 100007挂载5s)；500003=援护护盾-瞬发施法(2026-09-13 新增,AttackModeShieldCast:无弹道无伤害,当帧搜索同阵营最前排3个存活友军[排除自己]套伤害转移护盾2000700001,other_data 配 shield_count:3&shield_buff:2000700001,effect_hit=500003[Effect_Buff_Def_1加甲特效播BOSS出手点],sound_start=470001;大盾战士BOSS 1041020001 经ext 100005挂载[ext_type=2 全局技能,固定10s走路也放],替代原 creature_buff=2000600001 出生自带周期BUFF[BuffEntityPeriodicShieldCast 保留备用]) |
| `excel_attackmode_ext_info[攻击模块扩展信息].xlsx` | AttackModeExtInfo | 7 | id, attack_mode_id(对应AttackModeInfo), ext_type(额外类型:1=BOSS技能), trigger_interval(释放间隔秒,仅 scene=0 生效), trigger_scene(触发场景:0=攻击意图内释放[默认,AIIntentCreatureAttack 额外攻击机制消费],1=释放技能意图[NPC ai_param 的 skill_update 事件全局周期驱动,走路也放,间隔以 ai_param 为准]), remark （6列）。配 NpcInfo.attack_mode_ext 实现"额外攻击/技能"按间隔自动释放(不限于BOSS)；现有 100001=持盾战士BOSS前方1排挥砍(→102001,3s)、100002/100003=火/水大魔法师天降(→700001/700002,5s)、100004=大剑战士BOSS/勇者前方上中下3排挥砍(→102002,3s)、100005=大盾战士BOSS援护护盾(→500003,scene=1,大盾NPC ai_param=skill_update:100005:10 固定10s走路也放)、100006=冰大魔法师天降冰球(→700005,5s)、100007=雷大魔法师三连雷击(→700006,5s) |
...
| `excel_fight_type_conquer_info[战斗-征服模式].xlsx` | FightTypeConquerInfo | 12 | id, world_id, fight_scene_ids, enemy_ids, enemy_num, attack_wave, fight/road/level 参数, drop_crystal, reward_crystal(**string**: 单值"200"固定 或 区间"100-200"随机, 与其它区间字段同格式, 解析见 FightTypeConquerInfoBeanPartial.GetRandomRewardCrystal), reward_reputation(通关声望奖励) （24列） |
| `excel_fight_type_challenge_hundred_info[战斗-挑战100勇士].xlsx` | FightTypeChallengeHundredInfo | 28 | id, enemy_ids(进攻敌人npcInfoId池,生成100只怪每只独立随机), difficulty_levels(可抽中难度列表:世界最高已解锁难度在列才可抽中本行), challenge_type(0普通/1BOSS通关宝箱翻倍:装备件数x2/魔晶数量x2), attack_intensity_baserate(强度倍率,0/空按1,本模式强度自配不叠加议会强度议案), attack_show_time(进攻总时间秒,100只怪分桶均匀随机排布), road_num/road_length("x"单值或"x-y"区间), fight_scene_ids(随机其一), drop_crystal, reward_crystal(每档"x"或"x-y"区间随机,每箱), reward_equip_rarity, reward_exp(通关经验,阵容每只), remark （14列）。⚠️ 本表多值列统一用","分隔(征服表用"&",两表各自独立)；强度倍率/击杀掉晶/每箱魔晶/装备稀有度/通关经验为逐难度对齐字段(string 列:单值=全难度共用,或与难度列表等长逗号分隔按冻结难度取档)。一行=一个挑战配置(当前28行=征服全部进攻敌人14普通+14BOSS,难度=征服出场难度,数值随征服同难度档,BOSS经验取 reward_exp_boss)；Bean 扩展 FightTypeChallengeHundredInfoBeanPartial(GetEnemyIdList/GetRandomRow/GetIntensityRate(difficulty)/IsBossChallenge 等),消费方 GameWorldInfoRandomBean.SetRandomDataForChallengeHundred / FightBeanForChallengeHundred / RewardSelectBean.CreateRewardListForChallengeHundred / FightCreatureEntity.DropCrystal |
...
| 文件名 | Sheet | 数据行 | 主要列 |
|--------|-------|--------|--------|
| `excel_npc_info[NPC信息].xlsx` | NpcInfo | 181 | id, creature_id, npc_type, level, HP/MP/DR/ATK/ASPD/MSPD, skin_data, skin_color_data(皮肤固定颜色: `部位类型int,r,g,b,a&...` 如 `3,255,255,255,255`=Hair白色，rgba取0~255、部位类型见CreatureSkinTypeEnum、空=创建时随机染色), equip_item_ids, equip_random(随机装备: `池ID,稀有度1,稀有度2...` 如 `10000001,N,R`，池=CreatureRandomInfo的散件池type1/套装池type2，稀有度按RarityEnum枚举名、多个等概率抽、重复写加权、空=不随机；150条npc_type=3议员行已按评级配N~SSR), councilor_ratings, title_data, name[language], body_size(体型倍率: 空/0=1倍、"0.9,1.1"=区间随机、"1.1"=固定), attack_mode_ext(Boss额外技能:逗号分隔的AttackModeExtInfo id,非空即启用;触发场景由 ext 的 trigger_scene 列决定:0=攻击意图内释放,1=释放技能意图), ai_param(AI参数:`&`分隔多项,每项 `类型:参数...`;当前类型 `skill_update:技能extId:间隔秒`=注册AI Update事件每N秒触发指定技能[该技能须配 trigger_scene=1],多技能可并存,间隔以本参数为准;如 `skill_update:100005:10`;运行时由 AICreatureEntityForUpdateEvent(partial).InitUpdateEvents 解析注册,解析走 NpcInfoBeanPartial.GetListAIParamUpdateEvent[返回通用 AIParamUpdateEventBean:eventType+listParam]), remark, region(地区限制: 空=不限语言、语言代码如 `cn` 或 `cn,en`=仅这些语言下出现，多个英文逗号分隔，代码见LanguageEnum；目前仅终焉议会议员生成过滤生效), rarity(生物稀有度: 空/0=N、按RarityEnum枚举值1=N~6=L；配置后创建NPC生物时写入 CreatureBean.rarity，仅设置稀有度不授稀有度BUFF；150条npc_type=3随机议员行已按评级填充：评级1-2→N、3-4→R、5→SR，4条npc_type=2固定议员行配SSR), icon_res(头像图片: UI图集sprite名、支持「名,图集」后缀强制指定图集、空=用spine形象展示；无spine资源的NPC配置此字段，对话界面 UIGameConversation 据此走静态头像模式；当前仅监视之塔 id=10001 新手引导NPC 配 ui_book_1，该NPC creature_id=0无生物资源仅对话使用) （26列） |
...
| `excel_effect_info[粒子信息].xlsx` | EffectInfo | 31 | id, res_name, show_type, show_time, float/int/long/vector3/vector4_data, remark。2000001/2000002=火/水爆发(Effect_Boom_Fire_2/Water_2,原名 Effect_Burst_Fire_1/Water_1 于 2026-08-24 改名,火/水大魔法师BOSS技能 700001/700002 的 effect_hit 由 800001 火锥迁入;PS Burst 新粒子走全局单例通道,float/vector3 留空,位置由 ShowEnduringSingletonEffect 直接落点)；900004=落雷-BOSS(Effect_Thunder_4,2026-09-15 新增:雷大魔法师BOSS技能 700006 的子雷 800002 之 effect_hit 初始+连锁均用它,show_type=1/show_time空 照 2000001/2000002 BOSS技能击中先例,全局单例通道)；500004=护罩穹顶罩体资源(Effect_ShieldBubble_1:mesh+shader 视图非粒子,框架层通用组件 EffectShieldBubble[不继承 EffectBase]驱动碎裂/闪白/溶解;材质在游戏层 LoadResources/Materials/Effect/Mat_ShieldBubble_1.mat;预留备用) |
...
- **新增配置表的导出顺序（已知坑）**：`ExcelEditorWindow`「导出 Json」按 **表名+Bean 反射取类型**（`Assembly.Load("Assembly-CSharp").GetType(sheet+"Bean")`），新表必须**先「生成 Entity」并让 Unity 完成编译后再「导出 Json」**，否则该表被跳过（日志输出「未找到对应的实体类」）。首例：`excel_fight_type_challenge_hundred_info`（FightTypeChallengeHundredInfo）。
