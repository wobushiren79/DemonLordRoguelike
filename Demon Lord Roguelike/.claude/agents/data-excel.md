...
| `excel_fight_type_challenge_hundred_info[战斗-挑战100勇士].xlsx` | FightTypeChallengeHundredInfo | 2 | id, enemy_ids(进攻敌人npcInfoId池,生成100只怪每只独立随机), difficulty_levels(可抽中难度列表:世界最高已解锁难度在列才可抽中本行), attack_intensity_baserate(强度倍率,敌人HP/护甲/ATK×该值,0/空按1,本模式强度自配不叠加议会强度议案), attack_show_time(进攻总时间秒,100只怪分桶均匀随机排布), road_num/road_length("x"单值或"x-y"区间), fight_scene_ids(随机其一), drop_crystal, reward_crystal("x"或"x-y"区间随机,每箱), reward_equip_rarity, reward_exp(通关经验,阵容每只), remark （13列）。⚠️ 本表多值列统一用","分隔(征服表用"&",两表各自独立)。一行=一个挑战配置,difficulty_levels 可配多难度(当前2行=难度1-5/6-10,取各组最低档数值)；Bean 扩展 FightTypeChallengeHundredInfoBeanPartial(GetEnemyIdList/GetRandomRow 等),消费方 GameWorldInfoRandomBean.SetRandomDataForChallengeHundred / FightBeanForChallengeHundred |
