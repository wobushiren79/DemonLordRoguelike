...
- **UICreatureManager** - `RefreshSacrificeButton`：解锁祭坛且未满级时显示；经验达标则正常，经验未达标则 `SetSacrificeButtonGray` 置灰但仍可点击（点击走 `OnClickForCreatureSacrifice`，`!CanUpLevel()` 时 ToastHintText(textId 61009「经验值未达到100%」) 拦截）。未解锁祭坛/无选中生物/已满级(`IsMaxLevel()`)/魔王(`IsDemonLord()`,隐藏等级不吃经验故不可献祭)则隐藏；`OnClickForCreatureSacrifice` 开头对魔王 `return` 兜底拦截。背包道具点击 `EventForItemBackpackClickSelect` 经 `UseOrEquipItem` 分流：`ItemTypeEnum.Juice`(=11) → `UseJuiceItem`（#region 魔汁使用：确认弹窗 textId 61014 → 选中生物 `levelExp+=juicerExp`+消耗道具+落盘；满级 Toast 61015 拦截；魔王/无选中兜底）；`TransformPotion`(=18)/`RestorePotion`(=19) → `UseTransformPotionItem`/`UseRestorePotionItem`（幻化形象写入/清除 `CreatureBean.transformItemId`，含魔王可用，与献祭流程无关，详见 ui-game）；其余道具照旧走装备 `SetCreatureEquip`
