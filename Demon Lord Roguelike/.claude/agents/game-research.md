...
- **UserUnlockBean** - 玩家解锁数据（`unlockInfoData` 字典）
  ...
  - pre_data 前置解锁条件(特殊)：ResearchInfo 新列，格式 `条件枚举名:数值` 多条 `&` 组合(与, 数值缺省1)，条件枚举 `ResearchPreConditionEnum`(在 GameStateEnum.cs, 现有 World1ConquerCompleteCount1~10=世界1难度1~10通关次数)，拆分走通用扩展 `StringExtension.SplitForDictionaryEnumLong`、判定在 `ResearchInfoBeanPartial.GetPreDataConditions/CheckPreDataIsMeet`，与 pre_unlock_ids 一起决定节点显示；**pre_data 字段暂在 Partial，重新生成 Entity 后需删除该临时字段**
