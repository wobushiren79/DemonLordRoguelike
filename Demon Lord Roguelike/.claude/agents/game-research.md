...
- **ResearchInfoBeanPartial** - 扩展方法：前置解锁解析（`GetPreUnlockIdsForLine`）、类型枚举映射（`GetResearchType`）、阶梯水晶价格计算（`GetPayCrystal`）、带数值详情名称（`GetNameLanguageWithLevelDetail(currentLevel)`——仅对多语言模板含 `{Value}` 占位的节点(当前为 SpaceDash/SpaceDashCD)生效，气泡名称动态填「待解锁等级=min(当前+1,满级)」的距离/冷却数值（走通用 `TextHandler.GetTextReplace` + `TextReplaceEnum.Value`，与成就 `GetLevelDescription` 同口径），每级只显示当前要解锁那一级，满级停留满级数值）
