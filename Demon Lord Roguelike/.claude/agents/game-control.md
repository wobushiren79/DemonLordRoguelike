...
`None / CoreInteraction / PortalInteraction / DoomCouncilInteraction / DoomCouncilPodium / Councilor / AchievementInteraction`。交互物体 GameObject 名字即枚举名（带 `_UUID` 后缀的取下划线前段，如 `Councilor_xxx`），`Enum.TryParse` 失败回退 `None`。
