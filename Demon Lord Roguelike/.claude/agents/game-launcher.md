...
- **纯GUI测试面板清理收口**：`LauncherTest.ClearTestGUIs()`——销毁 `TestEffectGUI.Instance`/`TestCreatureCardGUI.Instance` 两个手工创建的纯 IMGUI 面板（挂在常驻启动场景，`ClearWorldData`/`UnLoadAllScene` 卸载不到）；**所有 `StartFor*` 测试入口首行统一调用**，切换测试模块时防旧面板与其场景残留（新增入口必须照做）。详见 test-system skill。
