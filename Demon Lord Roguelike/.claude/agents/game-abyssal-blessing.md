...
- **动态数值馈赠（当前用于 都是兄弟/杀红了眼，作用全体防守魔物）**：加成率随战况**每次重算属性时动态算**（非配置写死）。BUFF 继承抽象基类 `BuffEntityAttributeDynamicRate : BuffEntityAttribute`（重写 `CollectModifiers`+`ChangeData` 用 `GetDynamicRate()` 替代固定率、仅走 PercentAdd，用于 ATK/DR/HP；`trigger_creature_type=1` 不含核心）；子类为**通用功能类**（按缩放来源命名、不绑馈赠名，可被其它同功能馈赠复用）。6 族（每属性一族，均 `level=1~5` 升级链、`parent_id` 链式、`max_count=0`）：
  ...
  - 杀红了眼·攻击/护甲/生命：`BuffEntityAttributeScaleByKillCount`(data=ATK/DR/HP；通用"随本局累计击杀数缩放"，当前用于本馈赠)，`rate=fightRecordsData.totalKillNumForDef×trigger_value_rate`（仅魔物击杀、征服 run 内跨关卡累积）。馈赠 id `2000011001~005`/`2000012001~005`/`2000013001~005`，BUFF id `3001800001~005`/`3001900001~005`/`3002000001~005`，图标 `ui_abyssalblessing_123/124/125`。
