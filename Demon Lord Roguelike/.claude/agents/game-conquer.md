...
### 编辑器
- **FightTypeConquerEditorWindow** - 征服配置可视化编辑、保存回 Excel 并重导 JSON（反射按字段名）；数值字段左右分列对比前后各3个难度(level±1~±3)只读值、差异高亮，方便跨难度调数值；参数可复制（点任意对比单元格复制单字段 / 顶部按难度一键「复制全部数值」按钮，跳过 id/world_id/level，复制后仍需保存）
