...
- **开始进阶即移出阵容**：`OnClickForStart` 确认后除设 `creatureState=Vat` 外，还调 `userData.RemoveLineupCreature(uuid)` 把目标魔物移出阵容（进阶期间不可出战）；完成时 `UserAscendBean.RemoveAscendData` 把状态复位 `Idle`（仍在背包、不自动回阵容，玩家自行重新编入）。确认弹窗文案 80010 已含「进阶中的魔物将移出阵容」提示。素材魔物仍走 `RemoveAllEquipToBackpack()`+`RemoveBackpackCreature()`（装备退回背包后再删）。
