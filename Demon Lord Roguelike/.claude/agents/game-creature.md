...
- **show_attribute 展示属性配置（creature_info 列）** - 逗号分隔 `CreatureAttributeTypeEnum` 枚举值（如 `1,3,4,6`），`CreatureInfoBeanPartial.GetShowAttributeList()` 懒解析（空/解析失败兜底默认 HP/DR/ATK/ASPD）。同一配置同时控制三处：详情面板显示项（`UIViewCreatureCardDetails.SetAttribute`）、献祭加点可加项（`UICreatureAddAttribute.InitItems`）、创建随机加点池（`AddRandomAttributeForCreate(addNum, pool)`）。烂泥/毒液史莱姆(3003/3004)配 `4` 仅攻击力、守护史莱姆(3001)配 `1,3` 仅HP/DR；魔王物种行(id 1-7)配 `4,5,2,11`(ATK/MSPD/MP/MPF)，详情面板所有生物统一走配置无特判
...
- **最前排搜索（FindFrontRowCreatures）** - `FightCreatureSearchUtil.FindFrontRowCreatures(list, count, excludeCreature, fightType)`：按 x 坐标排名取前 N 个存活生物（**FightAttack 前排=x 最小=最靠近敌方阵地，FightDefense 前排=x 最大**——敌人从 +x 向 -x 推进），排除施法者，无存活返回 null；现任用于大盾战士 BOSS「援护护盾」选套盾目标（机制详见 buff-system / game-fight-system 的伤害转移拦截）
...
- **幻化药/幻原药（生物形象幻化，transformItemId）** - `CreatureBean.transformItemId`（long 持久化字段，0=无幻化；幻化药 `ItemTypeEnum.TransformPotion`=18 使用后写入道具 id、幻原药 RestorePotion=19 置0；随存档 JSON 序列化，旧存档默认0兼容；只存 id 实时查 ItemsInfoCfg=Mod 保底：Mod 移除配置失效→全路径自动回落原形象、装回自动恢复，幻原药只判 id==0 不读配置仍可清除）。`CreatureBeanPartial`（#region 幻化相关）：`GetTransformItemInfo()` 共用前置校验（0/配置缺失 `loggedMissingTransformIds` 每 id 一次 LogError/类型非幻化药防 Mod id 复用/other_data 空→null）；`ParseTransformOtherData` 解析组合格式 `chessRes,avatorRes|uiScale;x,y`；`GetTransformSpineRes()`（**全项目唯一形象解析入口**）只取 chess 段；`GetTransformUIShowSpineRes()` 取 avator 段（ui_show_spine 高清展示，可空）；`GetTransformUIShowData()` 取 `|` 第三段详情UI尺寸（格式同 ui_data_b）。**`CreatureHandler.SetCreatureData` 中枢换骨（生物展示核心行为）**：有幻化则 `resName` 顶替为幻化资源（**整骨替换非换肤**；`isUIShow=true` 且配了 avator 段时再改用高清资源），SkeletonAnimation/SkeletonGraphic 两分支 `ChangeSkeletonSkin` 包 `if (!hasTransform)` **整体跳过**（禁止改成传 null——其末尾 `SetSkin(空皮肤)` 会清掉幻化骨架默认外观）；缩放行不变（仍按原生物 `size_spine × GetBodySizeScale()`；详情UI尺寸由 `GameUIUtil.SetCreatureUIForDetails` 用幻化自带尺寸段替代 `ChangeUISizeForB`）。一处漏斗覆盖：详情UI/列表小图标/对话头像/破蛋/献祭/加点/终焉议会议员/基地自控魔王/战斗实体皮肤阶段。**优先级链 Portrait > 幻化 > 原形象**（Portrait 由 `GameUIUtil.SetCreatureUIForDetails` 在 SetCreatureData 之后再覆盖，卸下自动显现幻化；连续吃幻化药后者覆盖前者）。`ClearTempData` 重置0（防池化残留），`ClearFightTempData` 不动（战斗后幻化保留）。战斗侧 `GetFightCreatureObj` 新增可选参数 `resNameOverride`，防御/核心两调用点传 `GetTransformSpineRes()`、进攻敌人不传——详见 creature-system / game-fight-system SKILL；使用入口（UICreatureManager 幻化药/幻原药分流）见 creature-card-system SKILL；Mod 幻化药生产流程（AeonsEchoSpine）见 aeonsecho-spine-mod agent/skill
