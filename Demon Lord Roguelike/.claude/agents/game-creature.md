...
- **幻化药/幻原药（生物形象幻化，transformItemId）** - `CreatureBean.transformItemId`（long 持久化字段，0=无幻化；幻化药 `ItemTypeEnum.TransformPotion`=18 使用后写入道具 id、幻原药 RestorePotion=19 置0；随存档 JSON 序列化，旧存档默认0兼容；只存 id 实时查 ItemsInfoCfg=Mod 保底：Mod 移除配置失效→全路径自动回落原形象、装回自动恢复，幻原药只判 id==0 不读配置仍可清除）。`CreatureBeanPartial.GetTransformSpineRes()`（#region 幻化相关，**全项目唯一形象解析入口**）：0 / 配置缺失（静态 `loggedMissingTransformIds` 每 id 一次 LogError 防列表刷屏）/ 类型非幻化药（防 Mod id 复用）/ `other_data` 空 均返回 null，否则返回 `ItemsInfo.other_data`（Spine SkeletonDataAsset 的 Addressables 资源名）。**`CreatureHandler.SetCreatureData` 中枢换骨（生物展示核心行为）**：有幻化则 `resName` 顶替为幻化资源（**整骨替换非换肤**），SkeletonAnimation/SkeletonGraphic 两分支 `ChangeSkeletonSkin` 包 `if (!hasTransform)` **整体跳过**（禁止改成传 null——其末尾 `SetSkin(空皮肤)` 会清掉幻化骨架默认外观）；缩放行不变（仍按原生物 `size_spine × GetBodySizeScale()`）。一处漏斗覆盖：详情UI/列表小图标/对话头像/破蛋/献祭/加点/终焉议会议员/基地自控魔王/战斗实体皮肤阶段。**优先级链 Portrait > 幻化 > 原形象**（Portrait 由 `GameUIUtil.SetCreatureUIForDetails` 在 SetCreatureData 之后再覆盖，卸下自动显现幻化；连续吃幻化药后者覆盖前者）。`ClearTempData` 重置0（防池化残留），`ClearFightTempData` 不动（战斗后幻化保留）。战斗侧 `GetFightCreatureObj` 新增可选参数 `resNameOverride`，防御/核心两调用点传 `GetTransformSpineRes()`、进攻敌人不传——详见 creature-system / game-fight-system SKILL；使用入口（UICreatureManager 幻化药/幻原药分流）见 creature-card-system SKILL
